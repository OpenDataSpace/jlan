...
public class MultiThreadedUdpRpcDatagramHandler extends UdpRpcDatagramHandler implements RpcPacketHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(MultiThreadedUdpRpcDatagramHandler.class);
    ...
    protected class DatagramSender implements Runnable {
         ...
         *            String
         */
        public DatagramSender(final String name) {
                ...
                if (rpc != null) {
                    try {
                        ...
                        if (mi_shutdown == false) {
                            LOGGER.error("DatagramSender " + Thread.currentThread().getName() + ":", ex);
                        }
                    } finally {
     ...
     *            int
     */
    public MultiThreadedUdpRpcDatagramHandler(final String name, final String protocol, final RpcProcessor rpcServer, final NetworkServer server,
            final InetAddress addr, final int port, final int maxSize) {
     ...
     */
    @Override
    protected boolean processDatagram(final DatagramPacket pkt) throws IOException {
     ...
     *            RpcPacket
     */
    protected final void queueRpcRequest(final RpcPacket rpc) {
     ...
     */
    @Override
    public void sendRpcResponse(final RpcPacket rpc) throws IOException {
     ...
     *            int
     */
    public final void setPacketPool(final int smallSize, final int smallPool, final int largeSize, final int largePool) {
     ...
     *            int
     */
    public final void setPacketPool(final int poolSize) {
        // Create the packet pool, if not already initialized
        if (m_packetPool == null) {
            // Create the packet pool
     ...
     *            RpcPacketPool
     */
    public final void setPacketPool(final RpcPacketPool pktPool) {
     ...
     *            int
     */
    public final void setThreadPool(final int numThreads) {
     ...
     *
     * @param threadPool
     *            RpcRequestThreadPool
     */
    public final void setThreadPool(final RpcRequestThreadPool threadPool) {
