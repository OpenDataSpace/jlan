...
public class NFSSrvSession extends SrvSession {
    private static final Logger LOGGER = LoggerFactory.getLogger(NFSSrvSession.class);
    ...
    private final InetAddress m_remAddr;
    private final int m_remPort;
     ...
     *            int
     */
    public NFSSrvSession(final NetworkServer srv, final InetAddress addr, final int port, final int type) {
    ...
    public final NetworkFileCache getFileCache() {
        ...
        if (m_fileCache == null) {
            ...
            final NFSConfigSection config = getNFSServer().getNFSConfiguration();
     ...
     *            ClientInfo
     */
    public final void setNFSClientInformation(final ClientInfo cInfo) {
     ...
     */
    public final TreeConnection findConnection(final int shareHash) {
     ...
     *            TreeConnection
     */
    public final void addConnection(final TreeConnection tree) {
        if (m_connections == null) {
     ...
     *            TreeConnection
     */
    public final void removeConnection(final TreeConnection tree) {
        if (m_connections == null) {
            return;
        }
     ...
     *            Object
     */
    public final void setAuthIdentifier(final Object authIdent) {
     ...
     *            long
     */
    public final void setLastAccess(final long dateTime) {
     ...
     */
    protected synchronized final int allocateSearchSlot(final SearchContext search) {
        ...
        if (idx == m_search.length) {
            ...
            final SearchContext[] newSearch = new SearchContext[m_search.length * 2];
     ...
     *            int
     */
    protected synchronized final void deallocateSearchSlot(final int ctxId) {
     ...
     *            int
     */
    protected final SearchContext getSearchContext(final int srchId) {
     ...
     *            SearchContext
     */
    protected final void setSearchContext(final int slot, final SearchContext srch) {
    ...
    protected final void cleanupSession() {
        ...
        if (LOGGER.isInfoEnabled() && hasDebug(NFSServer.DBG_SESSION)) {
            LOGGER.info(MarkerFactory.getMarker(this.getUniqueId()),
                    "NFS Cleanup session, searches=" + getSearchCount() + ", files=" + (m_fileCache != null ? m_fileCache.numberOfEntries() : 0)
        ...
        if (m_connections != null && m_connections.numberOfEntries() > 0) {
            ...
            final Enumeration<TreeConnection> conns = m_connections.enumerateConnections();

            while (conns.hasMoreElements()) {

                // Get the current tree connection

                final TreeConnection tree = conns.nextElement();

                tree.closeConnection(this);

                // Inform the driver that the connection has been closed

                final DeviceInterface devIface = tree.getInterface();
                if (devIface != null) {
