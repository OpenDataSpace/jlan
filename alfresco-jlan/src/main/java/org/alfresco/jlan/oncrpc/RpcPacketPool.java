...
public class RpcPacketPool {
    private static final Logger LOGGER = LoggerFactory.getLogger(RpcPacketPool.class);
    // Constants
    //
    ...
    private final Vector<RpcPacket> m_smallPackets;
    private final Vector<RpcPacket> m_largePackets;
     ...
     *            int
     */
    public RpcPacketPool(final int smallSize, final int smallLimit, final int largeSize, final int largeLimit) {
        // Create the small/large packet lists
        m_smallPackets = new Vector<RpcPacket>();
        m_largePackets = new Vector<RpcPacket>();
     ...
     *            int
     */
    public RpcPacketPool(final int largeSize, final int largeLimit) {
        // Create the small/large packet lists
        m_smallPackets = new Vector<RpcPacket>();
        m_largePackets = new Vector<RpcPacket>();
     ...
     * @return RpcPacket
     */
    public final RpcPacket allocatePacket(final int reqSize) {
        ...
        if (reqSize <= m_smallPktSize) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("RpcPacketPool Allocated (small) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length + ", list=" + m_smallPackets.size() + "/"
                        + m_smallPktLimit);
            }
        } else {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("RpcPacketPool Allocated (large) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length + ", list=" + m_largePackets.size() + "/"
                        + m_largePktLimit);
            }
        }
     ...
     *            RpcPacket
     */
    public final void releasePacket(final RpcPacket pkt) {
        ...
        if (pkt.getBuffer().length >= m_largePktSize) {
            ...
            synchronized (m_largePackets) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("RpcPacketPool Released (large) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length + ", list=" + m_largePackets.size());
                }
            }
        } else {
            ...
            synchronized (m_smallPackets) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("RpcPacketPool Released (small) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length);
                }
            }
        }
    }
