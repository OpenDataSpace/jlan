...
public class NetworkFileCache {
    private static final Logger LOGGER = LoggerFactory.getLogger(NetworkFileCache.class);
    ...
    private final Hashtable<Integer, FileEntry> m_fileCache;
    ...
    protected class FileEntry {
         ...
         *            NFSSrvSession
         */
        public FileEntry(final NetworkFile file, final TreeConnection conn, final NFSSrvSession sess) {
            m_file = file;
            m_conn = conn;
            m_sess = sess;
         ...
         *            long
         */
        public final void updateTimeout(final long tmo) {
            m_timeout = tmo;
        }
         ...
         *            NFSSrvSession
         */
        public final void setSession(final NFSSrvSession sess) {
            m_sess = sess;
        }
    ...
    protected class FileExpiry implements Runnable {
         ...
         *            String
         */
        public FileExpiry(final String name) {
        ...
        public void run() {
            ...
            while (m_shutdown == false) {
                ...
                synchronized (m_fileCache) {
                    ...
                    final Enumeration<Integer> enm = m_fileCache.keys();

                    while (enm.hasMoreElements()) {
                        // Get the current key
                        final Integer fileId = enm.nextElement();

                        // Get the file entry and check if it has expired
                        final FileEntry fentry = m_fileCache.get(fileId);
                        if (fentry != null && fentry.getTimeout() < timeNow) {
                            // Get the network file
                            final NetworkFile netFile = fentry.getFile();
                            ...
                            if (netFile.hasIOPending()) {
                                ...
                                if (hasDebug()) {
                                    LOGGER.info("NFSFileExpiry: I/O pending file={}, fid={}", fentry.getFile().getFullName(), fileId);
                                }
                            } else {
                                ...
                                if (fentry.isClosed() == false && netFile != null) {
                                    ...
                                    try {
                                        ...
                                        if (netFile.allowsOpenCloseViaNetworkFile() == false) {
                                            ...
                                            fentry.markAsClosed();
                                            if (hasDebug()) {
                                                LOGGER.info("NFSFileExpiry: Marked as closed file={}, fid={} (cached)", fentry.getFile().getFullName(), fileId);
                                            }
                                        } else {
                                            ...
                                            if (hasDebug()) {
                                                LOGGER.info("NFSFileExpiry: Closed file={}, fid={} (cached)", fentry.getFile().getFullName(), fileId);
                                            }
                                        }
                                        ...
                                        if (hasDebug()) {
                                            LOGGER.info("Error closing file, fentry=" + fentry + ", ex=" + ex.getMessage(), ex);
                                        }
                                    }
                                } else {
                                    ...
                                    try {
                                        ...
                                        final DiskInterface disk = (DiskInterface) fentry.getConnection().getInterface();
                                        ...
                                        if (disk.fileExists(fentry.getSession(), fentry.getConnection(), netFile.getFullName()) != FileStatus.NotExist) {
                                            ...
                                            if (netFile.isClosed() == false) {
                                                ...
                                                if (hasDebug()) {
                                                    LOGGER.info("NFSFileExpiry: Closed file={}, fid={} (removed)", fentry.getFile().getFullName(), fileId);
                                                }
                                            } else if (hasDebug()) {
                                                LOGGER.info("NFSFileExpiry: File already closed, file={}, fid={}", fentry.getFile().getFullName(), fileId);
                                            }
                                        } else if (hasDebug()) {
                                            LOGGER.info("NFSFileExpiry: File deleted before close, {}", netFile.getFullName());
                                        }
                                        // Clear the user context, flush any active transaction
                                        if (fentry.getSession().hasTransaction()) {
                                            fentry.getSession().endTransaction();
     ...
     *            String
     */
    public NetworkFileCache(final String name) {
     ...
     *            NFSSrvSession
     */
    public synchronized final void addFile(final NetworkFile file, final TreeConnection conn, final NFSSrvSession sess) {
        synchronized (m_fileCache) {
     ...
     *            int
     */
    public synchronized final void removeFile(final int id) {

        // Create the search key

        final Integer fileId = new Integer(id);
     ...
     * @return NetworkFile
     */
    public synchronized final NetworkFile findFile(final int id, final SrvSession sess) {
        // Create the search key
        final Integer fileId = new Integer(id);
    ...
    public final void closeAllFiles() {
        // Enumerate the cache entries
        final Enumeration<Integer> keys = m_fileCache.keys();
        while (keys.hasMoreElements()) {
            // Get the current key and lookup the matching value
            final Integer key = keys.nextElement();
            final FileEntry entry = m_fileCache.get(key);
     ...
     *            long
     */
    public final void setIOTimer(final long ioTimer) {
        m_fileIOTmo = ioTimer;
    }
     ...
     *            long
     */
    public final void setCloseTimer(final long closeTimer) {
     ...
     *            RpcAuthenticator
     */
    public final void setRpcAuthenticator(final RpcAuthenticator auth) {
        m_authenticator = auth;
    }

    /**
     * Dump the cache entries to the debug logger
     */
