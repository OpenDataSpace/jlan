...
public class MountServer extends RpcNetworkServer implements RpcProcessor {
    private static final Logger LOGGER = LoggerFactory.getLogger(MountServer.class);
     ...
     *            ServerConfiguration
     */
    public MountServer(final ServerConfiguration config) {
        super("Mount", config);
    ...
    public void startServer() {
        try {
            ...
            // Start the UDP request listener is a separate thread
            ...
            final PortMapping[] mappings = new PortMapping[4];
    ...
    public RpcPacket processRpc(final RpcPacket rpc) throws IOException {
        ...
        final int procId = rpc.getProcedureId();

        NFSSrvSession sess = null;

        if ((version == Mount.VersionId1 && procId == Mount.ProcNull1) == false && (version == Mount.VersionId3 && procId == Mount.ProcNull3) == false) {
            try {
     ...
     * @return RpcPacket
     */
    private final RpcPacket procMount(final NFSSrvSession sess, final RpcPacket rpc, final int version) {
        // Get the request parameters
        final String mountPath = rpc.unpackString();

        LOGGER.debug("[Mount] Mount request from {} path={}", rpc.getClientDetails(), mountPath);

        // Allocate the file handle
        final byte[] handle = allocateFileHandle(version);

        // Mount the path
        final int sts = mountPath(sess, mountPath, handle);
        ...
        if (version == 1) {
            ...
            // Create an authentication flavors array
     ...
     * @return RpcPacket
     */
    private final RpcPacket procDump(final NFSSrvSession sess, final RpcPacket rpc, final int version) {
        ...
        if (activeList != null) {
            ...
            for (int i = 0; i < activeList.numberOfEntries(); i++) {
                ...
                final MountEntry mntEntry = activeList.getEntryAt(i);
     ...
     * @return RpcPacket
     */
    private final RpcPacket procUnMount(final NFSSrvSession sess, final RpcPacket rpc, final int version) {
        // Get the request parameters
        final String mountPath = rpc.unpackString();
        LOGGER.debug("[Mount] UnMount request from {} path={}", rpc.getClientDetails(), mountPath);
     ...
     * @return RpcPacket
     */
    private final RpcPacket procUnMountAll(final NFSSrvSession sess, final RpcPacket rpc, final int version) {
     ...
     * @return RpcPacket
     */
    private final RpcPacket procExport(final NFSSrvSession sess, final RpcPacket rpc, final int version) {
        ...
        if (sess.getServer().hasAccessControlManager()) {
            ...
            final AccessControlManager aclMgr = sess.getServer().getAccessControlManager();
        ...
        final Enumeration<SharedDevice> enm = shareList.enumerateShares();
     ...
     * @return RpcPacket
     */
    private final RpcPacket procExportAll(final NFSSrvSession sess, final RpcPacket rpc) {
        return null;
    }

    /**
     * Create a temporary session for a request. There is no need to cache sessions in the mount server as usually only single requests are made.
     *
     * @param rpc
     *            RpcPacket
     ...
     */
    private final NFSSrvSession createTemporarySession(final RpcPacket rpc) throws RpcAuthenticationException {
        // Authenticate the request
        final RpcAuthenticator rpcAuth = getNFSConfiguration().getRpcAuthenticator();
        final Object sessKey = rpcAuth.authenticateRpcClient(rpc.getCredentialsType(), rpc);

        // Create an NFS session for the request
        final NFSSrvSession nfsSess = new NFSSrvSession(this, rpc.getClientAddress(), rpc.getClientPort(), rpc.getClientProtocol());
     ...
     * @return int
     */
    protected final int mountPath(final NFSSrvSession sess, final String path, final byte[] handle) {
        ...
        if (path.startsWith(UNIX_SEPERATOR) && path.length() >= 2) {
            ...
            final int pos = path.indexOf(UNIX_SEPERATOR, 1);
            if (pos != -1) {
            ...
            if (share != null) {
                ...
                if (getSecurityConfiguration().hasAccessControlManager()) {
                    ...
                    final AccessControlManager aclMgr = getSecurityConfiguration().getAccessControlManager();

                    if (aclMgr.checkAccessControl(sess, share) == AccessControl.NoAccess) {
                        LOGGER.info("Failed to mount path={}, access denied", path);
                ...
                if (extraPath != null) {
                    ...
                    try {
                        ...
                        final TreeConnection conn = m_connections.findConnection(shareId);
                        final DiskInterface disk = (DiskInterface) conn.getSharedDevice().getInterface();

                        // Validate the path
                        final FileInfo finfo = disk.getFileInformation(sess, conn, extraPath);
                ...
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info("Mounted path={}, handle={}", path, NFSHandle.asString(handle));
                }
                ...
                LOGGER.info("Failed to mount path={}", path);
     ...
     * @return byte[]
     */
    private final byte[] allocateFileHandle(final int version) {
        byte[] handle = null;
        if (version == 1) {
