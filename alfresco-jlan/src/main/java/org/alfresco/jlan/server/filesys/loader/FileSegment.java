...
public class FileSegment {
    ...
    private final FileSegmentInfo m_info;
     ...
     *            boolean
     */
    public FileSegment(final FileSegmentInfo info, final boolean writeable) {
        m_info = info;
    ...
    public final boolean isDataLoading() {
        if (m_info.hasStatus() == FileSegmentInfo.Initial && m_info.isQueued() == false) {
            return false;
        }
        return true;
    }
    ...
    public final boolean isDataAvailable() {
        if (m_info.hasStatus() >= FileSegmentInfo.Available && m_info.hasStatus() < FileSegmentInfo.Error) {
            return true;
        }
        return false;
    }
     ...
     *            int
     */
    public final void setStatus(final int sts) {
     ...
     *            boolean
     */
    public final synchronized void setStatus(final int sts, final boolean queued) {
        m_info.setStatus(sts);
    ...
    /**
     * Get the load lock for this file. If successful the current thread will proceed and can load the file, else the thread will wait until the load has been
     * completed by the thread with the lock.
     *
     * @return boolean true if the current thread has the load lock, else false to indicate that the file should now be loaded.
     ...
     * @exception IOException
     */
    public synchronized final int readBytes(final byte[] buf, final int len, final int pos, final long fileOff) throws IOException {
        // Check if the temporary file is open
        if (m_segment == null) {
            // Open the temporary file
            openFile();
        ...
        try {
            ...
            LOGGER.error("***** FileSegment Read Error *****", ex);
        }
     ...
     * @exception IOException
     */
    public synchronized final void writeBytes(final byte[] buf, final int len, final int pos, final long fileOff) throws IOException {
        // Check if the temporary file is open
        if (m_segment == null) {
        ...
        final long fileLen = m_segment.length();
        final long endpos = fileOff + len;
     ...
     *            long
     */
    public final void truncate(final long siz) throws IOException {
        // Check if the temporary file is open
        if (m_segment == null) {
            openFile();
        }
     ...
     * @exception IOException
     */
    public final static FileSegment createSegment(final FileSegmentInfo info, final String prefix, final String fname, final File tempDir,
            final boolean writeable) throws IOException {
        // Create a temporary file for the data segment
        final File tempFile = File.createTempFile(prefix, fname, tempDir);
     ...
     * @exception IOException
     */
    public final static FileSegment createSegment(final FileSegmentInfo info, final String fname, final File tempDir, final boolean writeable)
            throws IOException {
        // Create a temporary file for the data segment
        final File tempFile = new File(tempDir, fname);
        info.setTemporaryFile(tempFile.getAbsolutePath());
