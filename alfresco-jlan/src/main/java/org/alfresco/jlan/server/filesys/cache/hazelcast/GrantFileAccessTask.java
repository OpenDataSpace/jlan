...
public class GrantFileAccessTask extends RemoteStateTask<FileAccessToken> {
    private static final Logger LOGGER = LoggerFactory.getLogger(GrantFileAccessTask.class);
     ...
     *            boolean
     */
    public GrantFileAccessTask(final String mapName, final String key, final GrantAccessParams params, final boolean debug, final boolean timingDebug) {
     ...
     *
     * @param stateCache
     *            IMap<String, HazelCastFileState>
     * @param fState
     *            HazelCastFileState
     ...
     */
    @Override
    protected FileAccessToken runRemoteTaskAgainstState(final IMap<String, ClusterFileState> stateCache, final ClusterFileState fState) throws Exception {
        if (hasDebug()) {
            LOGGER.debug("GrantFileAccessTask: Open params=" + m_params + " path " + fState);
        }
        ...
        if (m_params.isAttributesOnlyAccess()) {
            ...
            attribsOnly = true;
            if (hasDebug()) {
                LOGGER.debug("Attributes only access for " + fState);
            }
        } else if (fState.getOpenCount() > 0) {
            // Get the current primary owner details, the owner node name/port
            final String curPrimaryOwner = (String) fState.getPrimaryOwner();
            if (hasDebug()) {
                LOGGER.debug("File already open by " + curPrimaryOwner + ", pid=" + fState.getProcessId() + ", sharingMode="
                        + SharingMode.getSharingModeAsString(fState.getSharedAccess()));
            }
            ...
            if (m_params.getSecurityLevel() == WinNT.SecurityImpersonation && m_params.getProcessId() == fState.getProcessId()
                ...
                if (hasDebug()) {
                    LOGGER.debug("Sharing mode disallows write access path={}", fState.getPath());
                }
            }
            ...
            // Check if the required sharing mode is allowed by the current file open
                ...
                if (hasDebug()) {
                    LOGGER.debug("Local share mode=0x{}, params share mode=0x{}", Integer.toHexString(fState.getSharedAccess()),
                            Integer.toHexString(m_params.getSharedAccess()));
                }
            }
            ...
            if (hasDebug()) {
                LOGGER.debug("Granted oplock type={}", OpLock.getTypeAsString(grantedOplock));
            }
        }
        ...
        if (nosharing == true) {
            ...
            if (grantedOplock != OpLock.TypeNone) {
                try {
                    ...
                    final RemoteOpLockDetails remoteOplock = new RemoteOpLockDetails(m_params.getOwnerName(), grantedOplock, fState.getPath(), null);
                    fState.setOpLock(remoteOplock);
                } catch (final ExistingOpLockException ex) {
                    if (hasDebug()) {
                        LOGGER.debug("Failed to set oplock on {}, existing oplock={}", fState, fState.getOpLock());
                    }
        ...
        final HazelCastAccessToken hcToken = new HazelCastAccessToken(m_params.getOwnerName(), m_params.getProcessId(), grantedOplock, oplockNotAvailable);
