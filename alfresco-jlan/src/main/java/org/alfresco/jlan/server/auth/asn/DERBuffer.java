...
public class DERBuffer {
     ...
     *            int
     */
    public DERBuffer(final int siz) {
        m_data = new byte[siz];
        m_pos = 0;
        m_offset = 0;
    }
     ...
     *            int
     */
    public DERBuffer(final byte[] buf, final int off, final int len) {
        m_data = buf;
        m_offset = off;
        m_pos = off;
        m_endpos = off + len;
     ...
     *            byte[]
     */
    public DERBuffer(final byte[] buf) {
        m_data = buf;
        m_offset = 0;
        m_pos = 0;
        m_endpos = buf.length;
     ...
     * @return byte[]
     */
    public final byte[] unpackBytes(final int len) {
        // Check if there is enough data in the buffer
     ...
     * @return int
     */
    public final int unpackInt(final int len) {
        // Check if there is enough data in the buffer
        if (m_data.length - m_pos < len) {
            throw new ArrayIndexOutOfBoundsException("End of data buffer");
        }
     ...
     * @return long
     */
    public final long unpackLong(final int len) {
        // Check if there is enough data in the buffer
        if (m_data.length - m_pos < len) {
            throw new ArrayIndexOutOfBoundsException("End of data buffer");
        }
     ...
     *            int
     */
    public final void packByte(final int bval) {
     ...
     *            int
     */
    public final void packByteAt(final int pos, final int bval) {
        // Pack the byte value
     ...
     *            int
     */
    public final void packBytes(final byte[] buf, final int off, final int len) {
        // Check if there is enough space in the buffer
        if (m_data.length - m_pos < len) {
            extendBuffer(len);
        }
     ...
     *            DERBuffer
     */
    public final void packBytes(final DERBuffer buf) {
        // Check if there is enough space in the buffer
     ...
     *            int
     */
    public final void packLength(final int len) {
        // Check if there is enough space in the buffer
        ...
        if (len < 128) {
            ...
            final int sizByts = calculateLengthBytes(len);
     ...
     *            int
     */
    public final void packInt(final int ival) {
        // Check if there is enough space in the buffer
        if (m_data.length - m_pos < 4) {
            extendBuffer();
        }
     ...
     *            long
     */
    public final void packLong(final long lval) {
        // Check if there is enough space in the buffer
     ...
     * @throws IOException
     */
    public final void packObject(final DERObject derObj) throws IOException {
        ...
        if (derObj.isTagged()) {
            ...
            final DERBuffer derBuf = new DERBuffer();
     ...
     */
    public final void packApplicationSpecific(final DERObject derObj) throws IOException {
        // Pack the object
     ...
     * @exception IOException
     */
    public final void packApplicationSpecific(final int tagId, final DERObject derObj) throws IOException {
        // Pack the header
     ...
     * @exception IOException
     */
    public final void packApplicationSpecific(final List<DERObject> derList) throws IOException {
     ...
     * @exception IOException
     */
    public final void packApplicationSpecific(final int tagId, final List<DERObject> derList) throws IOException {
        // Pack the header
        packByte(DER.Application + DER.Constructed + (tagId & 0x1F));
        ...
        if (derList != null && derList.size() > 0) {
            ...
            for (int i = 0; i < derList.size(); i++) {
                ...
                final DERObject derObj = (DERObject) derList.get(i);
                derBuf.packObject(derObj);
            }
     ...
     * @exception IOException
     */
    public final void packApplicationSpecific(final int tagId, final byte[] byts) throws IOException {
        // Pack the header
        packByte(DER.Application + DER.Constructed + (tagId & 0x1F));
     ...
     * @exception IOException
     */
    public final void packApplicationSpecific(final byte[] byts) throws IOException {
        // Pack the bytes
     ...
     * @return int
     */
    public final int calculateLengthBytes(final int len) {
        // Check if the length will fit in a single byte
        int sizByts = 1;
