...
public class EnterpriseCifsAuthenticator extends CifsAuthenticator implements CallbackHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(EnterpriseCifsAuthenticator.class);
    // Constants
    //
    ...
    private static final int NTLM_FLAGS = NTLM.Flag56Bit + NTLM.Flag128Bit + NTLM.FlagLanManKey + NTLM.FlagNegotiateNTLM + NTLM.FlagNTLM2Key
    ...
    public void initialize(final ServerConfiguration config, final ConfigElement params) throws InvalidConfigurationException {
        ...
        if (params.getChild("disableSessionCleanup") != null) {
            ...
            if (hasDebug()) {
                LOGGER.info("[SMB] Disabled session cleanup (for virtual circuit zero logons)");
            }
        }
        ...
        final CIFSConfigSection cifsConfig = (CIFSConfigSection) config.getConfigSection(CIFSConfigSection.SectionName);

        // Check if Kerberos is enabled, get the Kerberos realm
        final ConfigElement krbRealm = params.getChild("Realm");
        if (krbRealm != null && krbRealm.getValue() != null && krbRealm.getValue().length() > 0) {
            ...
            final ConfigElement srvPassword = params.getChild("Password");
            ...
            final ConfigElement loginEntry = params.getChild("LoginEntry");
            if (loginEntry != null) {
            ...
            final ConfigElement principal = params.getChild("Principal");

            if (principal != null) {
                // Use the supplied principal name to build the account name
                final StringBuffer cifsAccount = new StringBuffer();
            ...
            if (hasDebug()) {
                LOGGER.info("[SMB] Using principal - " + m_accountName);
            }
            ...
            try {
                ...
                if (hasDebug()) {
                    LOGGER.info("[SMB] CIFS Kerberos authenticator error - " + ex.getMessage());
                }
            ...
            if (LOGGER.isDebugEnabled() && hasDebug()) {
                LOGGER.debug("[SMB] Enabling mechTypes :-");
            ...
            final Vector<Oid> mechTypes = new Vector<Oid>();
            ...
            try {
                ...
                final StringBuffer mic = new StringBuffer();
                ...
                // CIFS server accepts
                final NegTokenInit negTokenInit = new NegTokenInit(mechTypes, mecListMIC);

                // Encode the NegTokenInit blob
                m_negTokenInit = negTokenInit.encode();
            } catch (final IOException ex) {
                if (hasDebug()) {
                    LOGGER.info("[SMB] Error creating SPNEGO NegTokenInit blob - " + ex.getMessage());
                }
            ...
            final ConfigElement useSpnego = params.getChild("useSPNEGO");
            ...
            if (useSpnego != null) {
                ...
                try {
                    // Build the SPNEGO NegTokenInit that contains the authentication types that the
                    // CIFS server accepts
                    final NegTokenInit negTokenInit = new NegTokenInit(mechTypes, null);
        ...
        final ConfigElement disallowNTLMv1 = params.getChild("disallowNTLMv1");

        m_acceptNTLMv1 = disallowNTLMv1 != null ? false : true;

        // Check if ticket cracking should be enabled
        final ConfigElement enableTktCracking = params.getChild("enableTicketCracking");

        m_enableTicketCracking = enableTktCracking != null ? true : false;
        if (hasDebug() && m_enableTicketCracking == true) {
            LOGGER.info("[SMB] Kerberos ticket cracking enabled");
        }
    }
    ...
    public void handle(final Callback[] callbacks) throws IOException, UnsupportedCallbackException {
        ...
        for (final Callback callback : callbacks) {
            ...
            // Request for realm
                ...
                final RealmCallback cb = (RealmCallback) callback;
    ...
    public int getServerCapabilities() {
        return Capability.Unicode + Capability.RemoteAPIs + Capability.NTSMBs + Capability.NTFind + Capability.NTStatus + Capability.LargeFiles
                + Capability.LargeRead + Capability.LargeWrite + Capability.ExtendedSecurity + Capability.InfoPassthru + Capability.Level2Oplocks;
    }
     ...
     */
    @Override
    public void generateNegotiateResponse(final SMBSrvSession sess, final SMBSrvPacket respPkt, final boolean extendedSecurity) throws AuthenticatorException {
        ...
        final byte[] buf = respPkt.getBuffer();

        // Pack the CIFS server GUID into the negotiate response
        final UUID serverGUID = sess.getSMBServer().getServerGUID();
    ...
    public void processSessionSetup(final SMBSrvSession sess, final SMBSrvPacket reqPkt) throws SMBSrvException {
        ...
        if (reqPkt.getParameterCount() == 13) {
            try {
                ...
                return;
            } catch (final SMBSrvException ex) {
        ...
        final int maxBufSize = reqPkt.getParameter(2);
        final int maxMpx = reqPkt.getParameter(3);
        final int vcNum = reqPkt.getParameter(4);
        final int secBlobLen = reqPkt.getParameter(7);
        final int capabs = reqPkt.getParameterLong(10);
        ...
        if (hasDebug()) {
            LOGGER.info("[SMB] NT Session setup {}, MID={}, UID={}, PID={}", (useRawNTLMSSP() ? "NTLMSSP" : "SPNEGO"), reqPkt.getMultiplexId(),
                    reqPkt.getUserId(), reqPkt.getProcessId());
        }
        ...
        final ClientInfo client = ClientInfo.createInfo(null, null);
        ...
        final Object setupObj = sess.getSetupObject(client.getProcessId());
        ...
        if (sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
            if (respBlob == null) {
                LOGGER.info("[SMB] User " + client.getUserName() + " logged on " + (client != null ? " (type " + client.getLogonTypeString() + ")" : ""));
            } else {
                LOGGER.info("[SMB] Two stage logon (" + (isNTLMSSP ? "NTLMSSP" : "SPNEGO") + ")");
            }
        }

        // Get the response blob length, it can be null
        final int respLen = respBlob != null ? respBlob.length : 0;
        ...
        if (respBlob != null || sess.hasSetupObject(client.getProcessId()) || setupObj != null) {
            ...
            final int reqLen = respLen + 100; // allow for strings
            if (reqLen > respPkt.getAvailableLength()) {
                ...
                } catch (final NoPooledMemoryException ex) {
                    if (hasDebug()) {
                        LOGGER.debug("Authenticator failed to allocate packet from pool, reqSiz={}", respPkt.getByteOffset() + respLen);
                    }
        ...
        if (loggedOn == true) {
            ...
            if (vcNum == 0 && hasSessionCleanup()) {
                ...
                final int discCnt = sess.disconnectClientSessions();
                if (discCnt > 0 && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                    LOGGER.info("[SMB] Disconnected {} existing sessions from client, sess={}", discCnt, sess);
                }
            }
            ...
            final VirtualCircuit vc = new VirtualCircuit(vcNum, client);
            uid = sess.addVirtualCircuit(vc);

            if (uid == VirtualCircuit.InvalidUID) {
                if (sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                    LOGGER.info("[SMB] Failed to allocate UID for virtual circuit, {}", vc);
                }

                // Failed to allocate a UID
                throw new SMBSrvException(SMBStatus.NTTooManySessions, SMBStatus.SRVTooManyUIDs, SMBStatus.ErrSrv);
            } else if (sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                LOGGER.info("[SMB] Allocated UID={} for VC={}", uid, vc);
            }
        }
     ...
     * @exception SMBSrvException
     */
    private final byte[] doNtlmsspSessionSetup(final SMBSrvSession sess, final ClientInfo client, final byte[] secbuf, final int secpos, final int seclen,
            final boolean unicode) throws SMBSrvException {
        // Determine the NTLMSSP message type
        final int msgType = NTLMMessage.isNTLMType(secbuf, secpos);
        byte[] respBlob = null;

        if (msgType == -1) {
            if (LOGGER.isInfoEnabled() && hasDebug()) {
                LOGGER.info("[SMB] Invalid NTLMSSP token received");
                LOGGER.info("[SMB]   Token={}", HexDump.hexString(secbuf, secpos, seclen, " "));
            }
        ...
        // Check for a type 1 NTLMSSP message
            ...
            final Type1NTLMMessage type1Msg = new Type1NTLMMessage(secbuf, secpos, seclen);
            ...
            final NTLanManAuthContext ntlmCtx = new NTLanManAuthContext();

            // Build a type2 message to send back to the client, containing the challenge
            final String domain = sess.getSMBServer().getServerName();

            final List<TargetInfo> tList = new ArrayList<TargetInfo>();
            ...
            ntlmFlags = NTLM.FlagChallengeAccept + NTLM.FlagRequestTarget + NTLM.Flag128Bit + NTLM.FlagNegotiateNTLM + NTLM.FlagNegotiateUnicode
            ...
            final Type2NTLMMessage type2Msg = new Type2NTLMMessage();
            ...
            final Type3NTLMMessage type3Msg = new Type3NTLMMessage(secbuf, secpos, seclen, unicode);
            ...
            if (type3Msg.hasFlag(NTLM.Flag128Bit) && type3Msg.hasFlag(NTLM.FlagNTLM2Key)) {
                ...
                if (type3Msg.getNTLMHashLength() > 24) {
                    ...
                    if (hasDebug()) {
                        LOGGER.info("[SMB] Logged on using NTLMSSP/NTLMv2");
                    }
                } else {
                    // Looks like an NTLMv2 session key
                    doNTLMv2SessionKeyLogon(sess, client, type3Msg);
                    if (hasDebug()) {
                        LOGGER.info("[SMB] Logged on using NTLMSSP/NTLMv2SessKey");
                    }
                }
            } else {
                ...
                if (hasDebug()) {
                    LOGGER.info("[SMB] Logged on using NTLMSSP/NTLMv1");
                }
            }
        }
     ...
     * @exception SMBSrvException
     */
    private final byte[] doSpnegoSessionSetup(final SMBSrvSession sess, final ClientInfo client, final byte[] secbuf, final int secpos, final int seclen,
            final boolean unicode) throws SMBSrvException {
        ...
        if (tokType == SPNEGO.NegTokenTarg && sess.hasSetupObject(client.getProcessId())
            ...
            final NegTokenTarg negToken = new NegTokenTarg();
            try {
            ...
            final byte[] ntlmsspBlob = negToken.getResponseToken();

            // Perform an NTLMSSP session setup
            final byte[] ntlmsspRespBlob = doNtlmsspSessionSetup(sess, client, ntlmsspBlob, 0, ntlmsspBlob.length, unicode);
            ...
            final NegTokenInit negToken = new NegTokenInit();
            try {
                // Decode the security blob
                negToken.decode(secbuf, secpos, seclen);
            } catch (final IOException ex) {
            ...
            if (oidStr != null && oidStr.equals(OID.ID_NTLMSSP)) {
                ...
                final byte[] ntlmsspBlob = negToken.getMechtoken();
                ...
                if (LOGGER.isInfoEnabled() && hasDebug()) {
                    LOGGER.info("[SMB] No matching authentication OID found");
                    LOGGER.info("[SMB]   {}", negToken.toString());
                }
            ...
            if (hasDebug()) {
                LOGGER.info("[SMB] Unknown SPNEGO token type");
            }
        ...
        try {
            ...
            if (hasDebug()) {
                LOGGER.info("[SMB] Failed to encode NegTokenTarg - " + ex.getMessage());
            }
     ...
     * @exception SMBSrvException
     */
    private final NegTokenTarg doKerberosLogon(final SMBSrvSession sess, final NegTokenInit negToken, final ClientInfo client) throws SMBSrvException {
        ...
        try {
            ...
            final KerberosApReq krbApReq = new KerberosApReq();
            krbApReq.parseMechToken(negToken.getMechtoken());

            if (LOGGER.isDebugEnabled() && hasDebug()) {
                LOGGER.debug("[SMB] Kerberos AP-REQ - {}", krbApReq);
            }
            ...
            if (krbApReq.hasMutualAuthentication() && m_enableTicketCracking == true) {
                ...
                if (hasDebug()) {
                    LOGGER.debug("[SMB] Kerberos mutual auth required, parsing AP-REQ");
                }
                ...
                try {
                    ...
                    if (LOGGER.isDebugEnabled() && hasDebug()) {
                        LOGGER.debug("[SMB] Failed to parse AP-REQ", ex);
                    }
            ...
            final SessionSetupPrivilegedAction sessSetupAction = new SessionSetupPrivilegedAction(m_accountName, negToken.getMechtoken());
            final Object result = Subject.doAs(m_loginContext.getSubject(), sessSetupAction);
            ...
            if (result != null) {
                ...
                if (negToken.hasOid(OID.MSKERBEROS5)) {
                    ...
                    if (hasDebug()) {
                        LOGGER.debug("[SMB] Using OID MS Kerberos5 for NegTokenTarg");
                    }
                } else {
                    respOid = OID.KERBEROS5;
                    if (hasDebug()) {
                        LOGGER.debug("[SMB] Using OID Kerberos5 for NegTokenTarg");
                    }
                }
                ...
                if (krbAuthCtx != null) {
                    try {
                        ...
                        final byte[] respToken = krbAuthCtx.parseKerberosApRep(krbDetails.getResponseToken());
                        ...
                        if (hasDebug()) {
                            LOGGER.debug("[SMB] Created NegTokenTarg using updated AP-REP, added subkey");
                        }
                    } catch (final Exception ex) {
                        if (hasDebug()) {
                            LOGGER.debug("[SMB] AP-REP Error:", ex);
                        }
                    }
                } else {
                    ...
                    negTokenTarg = new NegTokenTarg(SPNEGO.AcceptCompleted, respOid, krbDetails.getResponseToken());
                    if (hasDebug()) {
                        LOGGER.debug("[SMB] Created NegTokenTarg using standard Krb5 API response");
                    }
                }

                // Check if this is a null logon
                final String userName = krbDetails.getUserName();
                if (userName != null) {
                    ...
                    if (userName.endsWith("$") && userName.equals(userName.toUpperCase())) {
                        ...
                        if (hasDebug()) {
                            LOGGER.debug("[SMB] Machine account logon, {}, as null logon", userName);
                        }
                    } else {
                ...
                sess.setLoggedOn(true);
                if (hasDebug()) {
                    LOGGER.debug("[SMB] Logged on using Kerberos, user {}", userName);
                }
            } else {
                if (hasDebug()) {
                    LOGGER.debug("[SMB] No SPNEGO response, Kerberos logon failed");
                }
            ...
            if (hasDebug()) {
                LOGGER.error("[SMB] Kerberos logon error", ex);
            }
     ...
     * @exception SMBSrvException
     */
    private final void doNTLMv1Logon(final SMBSrvSession sess, final ClientInfo client, final Type3NTLMMessage type3Msg) throws SMBSrvException {
        // Check if NTLMv1 logons are allowed
        if (acceptNTLMv1Logon() == false) {
            // NTLMv1 password hashes not accepted
            if (hasDebug()) {
                LOGGER.warn("[SMB] NTLMv1 not accepted, client {}", sess.getRemoteName());
            }
        ...
        sess.getSetupObject(client.getProcessId());
        sess.removeSetupObject(client.getProcessId());
        ...
        final UserAccount user = getUserDetails(userName);
        if (user != null) {
            // Authenticate the user
            final int sts = authenticateUser(client, sess, NTLM1);
            if (sts < 0) {
                // Logon failed
                throw new SMBSrvException(SMBStatus.NTLogonFailure, SMBStatus.ErrDos, SMBStatus.DOSAccessDenied);
            }
            ...
            if (hasDebug()) {
                LOGGER.error("[SMB] User does not exist, {}", userName);
            }
     ...
     * @exception SMBSrvException
     */
    private final void doNTLMv1Logon(final SMBSrvSession sess, final ClientInfo client) throws SMBSrvException {
        // Check if NTLMv1 logons are allowed
        if (acceptNTLMv1Logon() == false) {
            // NTLMv1 password hashes not accepted
            if (hasDebug()) {
     ...
     * @exception SMBSrvException
     */
    private final void doNTLMv2Logon(final SMBSrvSession sess, final ClientInfo client, final Type3NTLMMessage type3Msg) throws SMBSrvException {
        // Get the type 2 message that contains the challenge sent to the client
        final Type2NTLMMessage type2Msg = (Type2NTLMMessage) sess.getSetupObject(client.getProcessId());
        sess.removeSetupObject(client.getProcessId());
        ...
        if (user != null) {
            try {
                ...
                if (user.hasMD4Password()) {
                    ...
                    md4Pwd = getEncryptor().generateEncryptedPassword(user.getPassword(), type2Msg.getChallenge(), PasswordEncryptor.MD4, null, null);
                    user.setMD4Password(md4Pwd);
                }

                // Generate the v2 hash using the challenge that was sent to the client
                final byte[] v2hash = getEncryptor().doNTLM2Encryption(md4Pwd, type3Msg.getUserName(), type3Msg.getDomain());

                // Get the NTLMv2 blob sent by the client and the challenge that was sent by the
                // server
                final NTLMv2Blob v2blob = new NTLMv2Blob(type3Msg.getNTLMHash());
                final byte[] srvChallenge = type2Msg.getChallenge();

                // Calculate the HMAC of the received blob and compare
                final byte[] srvHmac = v2blob.calculateHMAC(srvChallenge, v2hash);
                final byte[] clientHmac = v2blob.getHMAC();
            ...
            if (hasDebug()) {
                LOGGER.info("[SMB] User does not exist, {}", userName);
            }
     ...
     * @exception SMBSrvException
     */
    private final void doNTLMv2Logon(final SMBSrvSession sess, final ClientInfo client) throws SMBSrvException {
        // Check for a null logon
        ...
        // Get the user details
        final UserAccount user = getUserDetails(client.getUserName());
        if (user != null) {
            try {
                ...
                if (sess.hasAuthenticationContext()) {
                    ...
                    final NTLanManAuthContext ntlmCtx = (NTLanManAuthContext) sess.getAuthenticationContext();
                ...
                if (user.hasMD4Password()) {
                    ...
                    md4Pwd = getEncryptor().generateEncryptedPassword(user.getPassword(), srvChallenge, PasswordEncryptor.MD4, null, null);
                    user.setMD4Password(md4Pwd);
                }

                // Create the NTLMv2 blob from the received hashed password bytes
                final NTLMv2Blob v2blob = new NTLMv2Blob(client.getPassword());

                // Generate the v2 hash using the challenge that was sent to the client
                final byte[] v2hash = getEncryptor().doNTLM2Encryption(md4Pwd, client.getUserName(), client.getDomain());
            ...
            if (hasDebug()) {
                LOGGER.info("[SMB] User does not exist, {}", client.getUserName());
            }
     ...
     * @exception SMBSrvException
     */
    private final void doNTLMv2SessionKeyLogon(final SMBSrvSession sess, final ClientInfo client, final Type3NTLMMessage type3Msg) throws SMBSrvException {
        // Get the type 2 message that contains the challenge sent to the client
        final Type2NTLMMessage type2Msg = (Type2NTLMMessage) sess.getSetupObject(client.getProcessId());
        sess.removeSetupObject(client.getProcessId());
        ...
        if (user != null) {
            ...
            final byte[] v2challenge = new byte[8];
            ...
            try {
                ...
                md5.update(nonce);
                final byte[] md5nonce = md5.digest();
            ...
            if (user.hasMD4Password()) {
                ...
                try {
                    md4byts = getEncryptor().generateEncryptedPassword(user.getPassword(), null, PasswordEncryptor.MD4, null, null);
                } catch (final Exception ex) {
                    if (hasDebug()) {
                        LOGGER.error(ex.getMessage(), ex);
                    }
            ...
            final byte[] clientHash = type3Msg.getNTLMHash();
     ...
     * @exception SMBSrvException
     */
    private final void doHashedPasswordLogon(final SMBSrvSession sess, final SMBSrvPacket reqPkt) throws SMBSrvException {
        // Check that the received packet looks like a valid NT session setup andX request
        ...
        final int maxMpx = reqPkt.getParameter(3);
        final int vcNum = reqPkt.getParameter(4);
        final int ascPwdLen = reqPkt.getParameter(7);
        final int uniPwdLen = reqPkt.getParameter(8);
        final int capabs = reqPkt.getParameterLong(11);
        ...
        final byte[] ascPwd = reqPkt.unpackBytes(ascPwdLen);
        final byte[] uniPwd = reqPkt.unpackBytes(uniPwdLen);

        // Extract the user name string
        final String user = reqPkt.unpackString(isUni);
        ...
        if (sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
            LOGGER.info("[SMB] NT Session setup from user=" + user + ", password=" + (uniPwd != null ? HexDump.hexString(uniPwd) : "none") + ", ANSIpwd="
                    + (ascPwd != null ? HexDump.hexString(ascPwd) : "none") + ", domain=" + domain + ", os=" + clientOS + ", VC=" + vcNum + ", maxBuf="
                    + maxBufSize + ", maxMpx=" + maxMpx + ", authCtx=" + sess.getAuthenticationContext());
            LOGGER.info("[SMB]   MID=" + reqPkt.getMultiplexId() + ", UID=" + reqPkt.getUserId() + ", PID=" + reqPkt.getProcessId());
        }
        ...
        // Create the client information and store in the session
        final ClientInfo client = ClientInfo.getFactory().createInfo(user, uniPwd);
        ...
        if (uniPwd != null) {
            if (uniPwd.length == 24) {
                ...
                if (hasDebug()) {
                    LOGGER.info("[SMB] Logged on using Hashed/NTLMv1");
                }
            } else if (uniPwd.length > 0) {
                // NTLMv2 blob
                doNTLMv2Logon(sess, client);
                if (hasDebug()) {
                    LOGGER.info("[SMB] Logged on using Hashed/NTLMv2");
                }
            }
        } else {
        ...
        if (client.isGuest()) {
            ...
            isGuest = true;
            if (sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                LOGGER.info("[SMB] User {}, logged on as guest", user);
            }
        }

        // Create a virtual circuit and allocate a UID to the new circuit
        final VirtualCircuit vc = new VirtualCircuit(vcNum, client);
        final int uid = sess.addVirtualCircuit(vc);
