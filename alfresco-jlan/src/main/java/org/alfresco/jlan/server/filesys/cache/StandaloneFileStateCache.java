...
public class StandaloneFileStateCache extends FileStateCache {
    private static final Logger LOGGER = LoggerFactory.getLogger(StandaloneFileStateCache.class);
    ...
    public void initializeCache(final ConfigElement config, final ServerConfiguration srvConfig) throws InvalidConfigurationException {
        ...
        final ConfigElement elem = config.getChild("initialSize");
     ...
     */
    @Override
    public final FileState findFileState(final String path) {
     ...
     */
    @Override
    public final FileState findFileState(final String path, final boolean create) {
     ...
     */
    @Override
    public final FileState findFileState(final String path, final boolean create, final int status) {
        FileState state = null;
        synchronized (m_stateCache) {
            // Find the required file state, if it exists
            state = m_stateCache.get(FileState.normalizePath(path, isCaseSensitive()));
     ...
     */
    @Override
    public final FileState removeFileState(final String path) {
        FileState state = null;
        synchronized (m_stateCache) {
     ...
     */
    @Override
    public final void renameFileState(final String newPath, final FileState state, final boolean isDir) {
        ...
        synchronized (m_stateCache) {
            ...
            if (isDir == true) {
                ...
                for (final String statePath : m_stateCache.keySet()) {
                    ...
                    if (statePath.length() > oldPath.length() && statePath.startsWith(oldPath)) {
                        ...
                        final FileState renState = m_stateCache.get(statePath);

                        renState.setFileStatus(FileStatus.NotExist);
                        renState.setFileId(FileState.UnknownFileId);
                        if (hasDebug()) {
                            LOGGER.info("++ Rename update {}", statePath);
                        }
                    }
                }
            }
    ...
    public final void removeAllFileStates() {
        ...
        synchronized (m_stateCache) {
            ...
            for (final FileState state : m_stateCache.values()) {
                // Check if there is a state listener
                if (hasStateListener()) {
                    getStateListener().fileStateClosed(state);
                }
                if (hasDebug()) {
                    LOGGER.info("++ Closed: " + state.getPath());
                }
            }
    ...
    public final int removeExpiredFileStates() {
        ...
        synchronized (m_stateCache) {
            ...
            // Enumerate the file state cache and remove expired file state objects
            final Iterator<Map.Entry<String, FileState>> enm = m_stateCache.entrySet().iterator();
            ...
            while (enm.hasNext()) {
                // Get the file state
                final Map.Entry<String, FileState> entry = enm.next();
                final FileState state = entry.getValue();
            ...
            if (hasDebugExpiredStates() && openCnt > 0) {
                LOGGER.debug("++ Open files {}", openCnt);
                dumpCache(false);
            }
     ...
     */
    @Override
    public final void dumpCache(final boolean dumpAttribs) {
        ...
        synchronized (m_stateCache) {
            ...
            for (final Map.Entry<String, FileState> entry : m_stateCache.entrySet()) {
     ...
     */
    public void requestOplockBreak(final String path, final OpLockDetails oplock) throws IOException {
