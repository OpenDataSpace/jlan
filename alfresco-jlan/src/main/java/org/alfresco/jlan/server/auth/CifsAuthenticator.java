 ...
 *
 * <p>
 * An authenticator is used by the CIFS server to authenticate users when in user level access mode and authenticate requests to connect to a share when in
 * share level access.
 *
 * @author gkspencer
 */
public abstract class CifsAuthenticator implements ICifsAuthenticator {
    private static final Logger LOGGER = LoggerFactory.getLogger(CifsAuthenticator.class);
    // Server access mode
    ...
    private final PasswordEncryptor m_encryptor = new PasswordEncryptor();
    ...
    protected String getServerVersionString(final SMBSrvSession sess) {
        final String ret = sess.getSMBServer().getCIFSConfiguration().getComment();
        return (ret.isEmpty() ? "Alfresco CIFS Server" : ret) + " " + sess.getServer().isVersion();
    }

    /**
     * @param config
     *            an accessor for the file server configuration sections
     */
    public void setConfig(final ServerConfigurationAccessor config) {
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.jlan.server.auth.ICifsAuthenticator#authenticateShareConnect(org.alfresco.jlan.server.auth.ClientInfo,
     * org.alfresco.jlan.server.core.SharedDevice, java.lang.String, org.alfresco.jlan.server.SrvSession)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.jlan.server.auth.ICifsAuthenticator#authenticateUser(org.alfresco.jlan.server.auth.ClientInfo, org.alfresco.jlan.server.SrvSession,
     * int)
     */
    @Override
    public int authenticateUser(final ClientInfo client, final SrvSession sess, final int alg) {
        ...
        final UserAccount userAcc = getUserDetails(client.getUserName());
        if (userAcc != null) {
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.jlan.server.auth.ICifsAuthenticator#authenticateUserPlainText(org.alfresco.jlan.server.auth.ClientInfo,
     * org.alfresco.jlan.server.SrvSession)
     */
    public final int authenticateUserPlainText(final ClientInfo client, final SrvSession sess) {
        ...
        final byte[] encPwd = generateEncryptedPassword(textPwd, authCtx.getChallenge(), NTLM1, client.getUserName(), client.getDomain());
     ...
     * @return byte[]
     */
    protected final byte[] generateEncryptedPassword(final String plainPwd, final byte[] encryptKey, final int alg, final String userName,
            final String domain) {
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.jlan.server.auth.ICifsAuthenticator#generateNegotiateResponse(org.alfresco.jlan.smb.server.SMBSrvSession,
    ...
    public void generateNegotiateResponse(final SMBSrvSession sess, final SMBSrvPacket respPkt, final boolean extendedSecurity) throws AuthenticatorException {
        ...
        final ChallengeAuthContext authCtx = (ChallengeAuthContext) getAuthContext(sess);
        ...
        if (authCtx == null || authCtx.getChallenge() == null) {
            ...
            final byte[] key = authCtx.getChallenge();
        ...
        final String domain = sess.getSMBServer().getCIFSConfiguration().getDomainName();
        if (domain != null) {
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.jlan.server.auth.ICifsAuthenticator#processSessionSetup(org.alfresco.jlan.smb.server.SMBSrvSession,
    ...
    public void processSessionSetup(final SMBSrvSession sess, final SMBSrvPacket reqPkt) throws SMBSrvException {
        ...
        final int sts = authenticateUser(client, sess, ICifsAuthenticator.NTLM1);
        ...
        if (sts > 0 && (sts & ICifsAuthenticator.AUTH_GUEST) != 0) {
            ...
            if (LOGGER.isInfoEnabled() && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                LOGGER.info("[SMB] User " + user + ", logged on as guest");
            }
        } else if (sts != ICifsAuthenticator.AUTH_ALLOW) {
            if (LOGGER.isInfoEnabled() && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                LOGGER.info("[SMB] User " + user + ", access denied");
            }
        ...
        } else if (LOGGER.isInfoEnabled() && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
            LOGGER.info("[SMB] User " + user + " logged on " + (client != null ? " (type " + client.getLogonTypeString() + ")" : ""));
        }
        ...
        if (uid == VirtualCircuit.InvalidUID) {
            ...
            LOGGER.info("[SMB] Allocated UID=" + uid + " for VC=" + vc);
        }
        ...
        if (vcNum == 0 && hasSessionCleanup()) {
            ...
            if (discCnt > 0 && LOGGER.isInfoEnabled() && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
                LOGGER.info("[SMB] Disconnected " + discCnt + " existing sessions from client, sess=" + sess);
            }
        }

        // Check if there is a chained command with the session setup request (usually a TreeConnect)
     ...
     *
     * @param ena
     *            Enable the guest account if true, only allow defined user accounts access if false
     */
    public final void setAllowGuest(final boolean ena) {
        m_allowGuest = ena;
    }
     ...
     *            String
     */
    public final void setGuestUserName(final String guest) {
     ...
     *
     * @param ena
     *            Enable mapping of unknown users to the guest if true
     */
    public final void setMapToGuest(final boolean ena) {
        m_mapToGuest = ena;
    }
     ...
     *            int
     */
    protected final void setSecurityMode(final int flg) {
        m_securityMode = flg;
    }
     ...
     *            boolean
     */
    protected final void setExtendedSecurity(final boolean extSec) {
     ...
     *            boolean
     */
    public void setSessionCleanup(final boolean ena) {
        m_sessCleanup = ena;
    }
    ...
    /**
     * Validate a password by encrypting the plain text password using the specified encryption key and encryption algorithm.
     *
     * @param user
     *            UserAccount
     * @param client
     ...
     * @return boolean
     */
    protected final boolean validatePassword(final UserAccount user, final ClientInfo client, final AuthContext authCtx, final int alg) {
        // Get the challenge
        byte[] encryptKey = null;

        if (authCtx != null && authCtx instanceof NTLanManAuthContext) {
            // Get the NT/LanMan challenge
            final NTLanManAuthContext ntlmCtx = (NTLanManAuthContext) authCtx;
    ...
    protected final byte[] convertPassword(final String pwd) {
     ...
     * @return String
     */
    protected final String getStatusAsString(final int sts) {
        String str = null;
        switch (sts) {
            case AUTH_ALLOW:
                str = "Allow";
     ...
     *
     * @param mode
     *            Either SHARE_MODE or USER_MODE.
     */
    public final void setAccessMode(final int mode) {
        m_accessMode = mode;
    }
     ...
     *            SrvSession
     */
    protected void doGuestLogon(final ClientInfo client, final SrvSession sess) {
     ...
     */
    public final UserAccount getUserDetails(final String user) {
     ...
     * @return String
     */
    protected final String mapClientAddressToDomain(final InetAddress clientIP) {
        // Check if there are any domain mappings
        final SecurityConfigSection securityConfig = getsecurityConfig();
        if (securityConfig.hasDomainMappings() == false) {
            return null;
        }

        // Convert the client IP address to an integer value
        final int clientAddr = IPAddress.asInteger(clientIP);

        for (final DomainMapping domainMap : securityConfig.getDomainMappings()) {
            if (domainMap.isMemberOfDomain(clientAddr)) {
                if (LOGGER.isInfoEnabled() && hasDebug()) {
                    LOGGER.info("Mapped client IP " + clientIP + " to domain " + domainMap.getDomain());
                }
        ...
        if (LOGGER.isInfoEnabled() && hasDebug()) {
            LOGGER.info("Failed to map client IP " + clientIP + " to a domain");
        }
    ...
    public String toString() {
        return getClass().getName() + ", mode=" + (getAccessMode() == ICifsAuthenticator.SHARE_MODE ? "SHARE" : "USER");
    }
}
