 ...
 *
 * <p>
 * Contains a list of one or more servers that are used for passthru authentication. The status of the servers is tracked so that offline servers are not used
 * but periodically monitored so that they can be returned to the online list of servers.
 *
 * <p>
 * The server list may be initialized from a list of server names or addresses, or by specifying a domain name in which case the primary and backup domain
...
public class PassthruServers {
    private static final Logger LOGGER = LoggerFactory.getLogger(PassthruServers.class);
        ...
        public void run() {
            ...
            while (m_ishutdown == false) {
                ...
                if (getOfflineServerCount() > 0) {
                    ...
                    final PCShare authShare = new PCShare("", "IPC$", "", "");
     ...
     *            In seconds
     */
    public PassthruServers(final int checkInterval) {

        // Set the offline checker wakeup interval

        m_offlineCheckInterval = (checkInterval) * 1000L;
     ...
     * @return AuthenticateSession
     */
    public final AuthenticateSession openSession(final boolean useExtSec, final String clientDomain) {
        ...
        while (authSess == null && passthruServer != null && hasOnlineServers()) {
            final PCShare authShare = new PCShare(passthruServer.getAddress().getHostAddress(), "IPC$", "", "");
            try {
        ...
        if (getNullDomainUseAnyServer()) {
            ...
            synchronized (m_onlineList) {
                ...
                while (idx < m_onlineList.size() && passthruServer == null) {
                    ...
                    final PassthruServerDetails curServer = m_onlineList.get(idx);
    ...
    /**
     * Return the details of an online server to use for authentication of the specified client domain
     *
     * @param clientDomain
     *            String
     * @return PassthruServerDetails
     */
    protected PassthruServerDetails getAuthenticationServer(final String clientDomain) {
        // Rotate the head of the list and return the new head of list server details
        PassthruServerDetails passthruServer = null;
        ...
        if (passthruServer == null) {
            LOGGER.warn("No server found for domain {}", clientDomain);
        }
        return passthruServer;
    }
     ...
     */
    protected final void serverOffline(final PassthruServerDetails server) {
     ...
     *            PassthruServerDetails
     */
    protected final void serverOnline(final PassthruServerDetails server) {
        // Set the server status
     ...
     *            int
     */
    public final void setConnectionTimeout(final int tmo) {
        m_tmo = tmo;
    }
     ...
     *            long
     */
    public final void setOfflineCheckInterval(final long interval) {
    ...
    /**
     * Set the list of servers to use for passthru authentication using a comma delimited list of server names/addresses
     *
     * @param servers
     *            String
     */
    public final void setServerList(final String servers) {
        // Split the server list into separate name/address tokens
        final StringTokenizer tokens = new StringTokenizer(servers, ",");
        ...
        while (tokens.hasMoreTokens()) {
            ...
            if (srvName != null && srvAddr != null) {
                ...
                final PassthruServerDetails passthruServer = new PassthruServerDetails(srvName, domain, srvAddr, false);
     ...
     */
    public final void setDomain(final String domain) throws IOException {
        ...
        try {
            ...
            LOGGER.debug("  Found {} domain controller(s)", nbName.numberOfAddresses());
        ...
        for (int i = 0; i < nbName.numberOfAddresses(); i++) {
            ...
            try {
                ...
                final NetBIOSNameList nameList = NetBIOSSession.FindNamesForAddress(dcAddr.getHostAddress());
                final NetBIOSName dcNBName = nameList.findName(NetBIOSName.FileServer, false);
                ...
                dcName = dcAddr != null ? dcAddr.getHostAddress() : null;
            ...
            if (dcAddr != null) {
                ...
                final PassthruServerDetails passthruServer = new PassthruServerDetails(dcName, domain, dcAddr, false);
                m_offlineList.add(passthruServer);
                LOGGER.debug("Added passthru server {}", passthruServer);
            }
        }
     ...
     *            boolean
     */
    public final void setNullDomainUseAnyServer(final boolean nullDomain) {
