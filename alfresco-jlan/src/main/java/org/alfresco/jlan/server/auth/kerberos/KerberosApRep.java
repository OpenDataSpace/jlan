...
public class KerberosApRep {
     ...
     */
    public KerberosApRep(final byte[] blob) throws IOException {
        parseApRep(blob);
    }
     ...
     *            int
     */
    public final void setEncryptedPart(final int encType, final byte[] encData, final int encVno) {
        m_encType = encType;
        m_encData = encData;
        m_encKvno = encVno;
    }
     ...
     * @exception IOException
     */
    private final void parseApRep(final byte[] blob) throws IOException {
     ...
     * @exception IOException
     */
    public final void parseResponseToken(final byte[] respToken) throws IOException {
        // Create a stream to parse the ASN.1 encoded response token

        final DERBuffer derBuf = new DERBuffer(respToken);
        byte[] aprepBlob = null;
        ...
        if (appByts != null) {
            ...
            final DERBuffer appBuf = new DERBuffer(appByts);
    ...
    public final byte[] encodeApRep() throws IOException {
        ...
        final DERSequence aprepSeq = new DERSequence(2);
