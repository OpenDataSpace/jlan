...
public abstract class JdbcDBInterface implements DBInterface, DBConnectionPoolListener {
     ...
     */
    @Override
    public final boolean supportsFeature(final int feature) {
     ...
     */
    @Override
    public void requestFeatures(final int featureMask) throws DBException {

        // Get the database interface supported features

        final int supFeatures = getSupportedFeatures();
     ...
     *            String
     */
    protected final void setLockFile(final String lockFile) {
        m_lockFile = lockFile;
    }
     ...
     * @return long
     */
    protected final long setDataFragmentSize(final long fragSize) {
     ...
     */
    @Override
    public void initializeDatabase(final DBDeviceContext context, final ConfigElement params) throws InvalidConfigurationException {
        ...
        if (nameVal != null) {
            try {
                ...
                final String numVal = nameVal.getValue();
                final int pos = numVal.indexOf(':');
                ...
                if (m_dbInitConns < DBConnectionPool.MinimumConnections || m_dbInitConns > m_dbMaxConns) {
                    throw new InvalidConfigurationException("Database interface invalid initial connections value");
                }

                if (m_dbMaxConns > DBConnectionPool.MaximumConnections || m_dbMaxConns < m_dbInitConns) {
    ...
    public boolean isOnline() {
        ...
        if (m_connPool != null) {
            ...
            final Connection conn = m_connPool.getConnection();
            if (conn != null) {
     ...
     * @exception SQLException
     */
    protected final Connection getConnection(final long leaseTime) throws SQLException {

        // Get a database connection

        final Connection conn = m_connPool.getConnection(leaseTime);
        if (conn == null) {
            throw new SQLException("Failed to get database connection");
        }
     ...
     *            Connection
     */
    protected final void releaseConnection(final Connection conn) {
    ...
    /**
     * Return the data fragment size to store per BLOB when using the database to store the file data.
     *
     * @return long
     */
     ...
     *            String
     */
    protected final void setDSNString(final String dsn) {
        m_dsn = dsn;
    }
     ...
     *            String
     */
    protected final void setDriverName(final String driverClass) {
        m_driver = driverClass;
    }

    /**
     * Check for special characters within a file/directory name and escape the characters to return a string which can be used in a SQL statement.
     *
     * @param name
     *            String
     * @return String
     */
    protected String checkNameForSpecialChars(final String name) {
        ...
        final StringBuffer nameBuf = new StringBuffer(name.length() * 2);
     ...
     *            FileRequest
     */
    protected synchronized final void queueOfflineSaveRequest(final FileRequest saveReq) {
     ...
     */
    @Override
    public void databaseOnlineStatus(final boolean dbonline) {
        ...
        if (dbonline == true) {
            ...
            if (m_dbCtx.hasOfflineFileDeletes()) {
                ...
                final FileRequestQueue deleteList = m_dbCtx.getOfflineFileDeletes(true);

                for (int i = 0; i < deleteList.numberOfRequests(); i++) {

                    // Get the current delete file request

                    final DeleteFileRequest deleteReq = (DeleteFileRequest) deleteList.removeRequestNoWait();
            ...
            if (m_pendingSaveRequests != null) {
                ...
                final FileRequestQueue saveReqQueue = m_pendingSaveRequests;
                    ...
                    try {
                        ...
                        if (fileReq instanceof SingleFileRequest) {
                            ...
                            if (singleReq.hasFileState()) {
                                ...
                                final File tempFile = new File(singleReq.getTemporaryFile());

                                // Create the set file information details to set the file size

                                final FileInfo fInfo = new FileInfo("", 0L, 0);
    ...
    /**
     * Default implementation of the queued request check, throws an exception indicating that the feature is not implemented.
     *
     * @param tempFile
     *            String
     * @return boolean
     * @throws DBException
     */
    public boolean hasQueuedRequest(final String tempFile) throws DBException {
    ...
    /**
     * Default implementation of load file requests, throws an exception indicating that the feature is not implemented.
     *
     * @param seqNo
     *            int
     * @param reqType
     ...
     * @throws DBException
     */
    public int loadFileRequests(final int seqNo, final int reqType, final FileRequestQueue reqQueue, final int recLimit) throws DBException {
    ...
    /**
     * Default implementation of queue file request, throws an exception indicating that the feature is not implemented.
     *
     * @param fileReq
     *            FileRequest
     * @throws DBException
     */
    public void queueFileRequest(final FileRequest fileReq) throws DBException {
    ...
    /**
     * Default implementation of the queue transaction request, throws an exception indicating that the feature is not implemented.
     *
     * @param fileReq
     *            FileRequest
     * @exception DBException
     */
    public void queueTransactionRequest(final FileRequest fileReq) throws DBException {
    ...
    /**
     * Default implementation of load transaction request, throws an exception indicating that the feature is not implemented.
     *
     * @param tranReq
     *            MultipleFileRequest
