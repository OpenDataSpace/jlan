...
public class EncApRepPart {
     ...
     */
    public EncApRepPart(final byte[] blob) throws IOException {
        parseApRep(blob);
    }
     ...
     *            byte[]
     */
    public final void setSubkey(final int keyType, final byte[] subkey) {
     ...
     * @exception IOException
     */
    public final void parseApRep(final byte[] apRep) throws IOException {

        // Create a stream to parse the ASN.1 encoded blob

        final DERBuffer derBuf = new DERBuffer(apRep);
    ...
    public final byte[] encodeApRep() throws IOException {
        ...
        final DERSequence derList = new DERSequence();

        // Add the Kerberos time

        final DERGeneralizedTime derTime = new DERGeneralizedTime(getTimestamp());
        derTime.setTagNo(0);
        derList.addObject(derTime);

        // Add the microseconds

        final DERInteger derInt = new DERInteger(m_microseconds);
        derInt.setTagNo(1);
        derList.addObject(derInt);

        // Create the encryption key value

        final DERSequence derSeq = new DERSequence(2);
        ...
        final DERInteger derIntSeq = new DERInteger(m_seqNo);
        derIntSeq.setTagNo(3);
        derList.addObject(derIntSeq);

        // Pack the objects

        final DERBuffer derBuf = new DERBuffer(256);
