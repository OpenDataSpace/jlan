...
public class PassthruAuthenticator extends CifsAuthenticator implements SessionListener {
    private static final Logger LOGGER = LoggerFactory.getLogger(PassthruAuthenticator.class);
    ...
    private static final int NTLM_FLAGS = NTLM.Flag56Bit + NTLM.Flag128Bit + NTLM.FlagLanManKey + NTLM.FlagNegotiateNTLM + NTLM.FlagNegotiateUnicode;
    ...
    private final Hashtable<String, PassthruDetails> m_sessions;
     ...
     */
    @Override
    public int authenticateShareConnect(final ClientInfo client, final SharedDevice share, final String sharePwd, final SrvSession sess) {
    ...
    public int authenticateUser(final ClientInfo client, final SrvSession sess, final int alg) {
        ...
        if (client.isGuest() || client.getUserName().equalsIgnoreCase(getGuestUserName())) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Authenticated user {} sts={}", client.getUserName(), getStatusAsString(authSts));
            }
        ...
        final PassthruDetails passDetails = m_sessions.get(sess.getUniqueId());
        ...
        if (passDetails != null) {
            try {
                ...
                final AuthenticateSession authSess = passDetails.getAuthenticateSession();
                ...
                if (authSess.isGuest()) {
                    ...
                    if (allowGuest() == true) {
                        ...
                        LOGGER.debug("Passthru authenticate user={}, GUEST", client.getUserName());
                    }
                } else {
                    // Allow the user full access to the server
                    authSts = ICifsAuthenticator.AUTH_ALLOW;
                    LOGGER.debug("Passthru authenticate user={}, FULL", client.getUserName());
                }
            ...
            if ((sess instanceof SMBSrvSession) == false) {
                ...
                try {
                    ...
                    LOGGER.warn("Passthru error closing session (auth user) {}", ex.getMessage());
                }
            }
        } else {
    ...
    public AuthContext getAuthContext(final SMBSrvSession sess) {
        ...
        try {
            final AuthenticateSession authSess = m_passthruServers.openSession();
            if (authSess != null) {
                // Create an entry in the active sessions table for the new session
                final PassthruDetails passDetails = new PassthruDetails(sess, authSess);
                ...
                LOGGER.debug("Passthru sessId={}, auth ctx={}", authSess.getSessionId(), authCtx);
            }
        } catch (final Exception ex) {
            LOGGER.warn("Passthru error getting challenge " + ex.getMessage());
        }
    ...
    public void processSessionSetup(final SMBSrvSession sess, final SMBSrvPacket reqPkt) throws SMBSrvException {
        ...
        final ClientInfo client = ClientInfo.createInfo("", null);
        ...
        try {
            ...
            if (isNTLMSSP == true) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("NT Session setup NTLMSSP, MID={}, UID={}, PID={}", reqPkt.getMultiplexId(), reqPkt.getUserId(), reqPkt.getProcessId());
                }
        ...
        if (LOGGER.isInfoEnabled() && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE)) {
            if (respBlob == null) {
        ...
        if (loggedOn == true) {
            ...
            if (uid == VirtualCircuit.InvalidUID) {
                ...
                LOGGER.debug("Allocated UID={} for VC={}", uid, vc);
            }
        }
    ...
    private final byte[] doNtlmsspSessionSetup(final SMBSrvSession sess, final ClientInfo client, final byte[] secbuf, final int secpos, final int seclen,
        ...
        // Check for a type 1 NTLMSSP message
            ...
            final NTLanManAuthContext ntlmCtx = (NTLanManAuthContext) getAuthContext(sess);
            ...
            ntlmFlags = NTLM.FlagChallengeAccept + NTLM.FlagRequestTarget + NTLM.FlagNegotiateNTLM + NTLM.FlagNegotiateUnicode + NTLM.FlagKeyExchange
                    + NTLM.FlagTargetInfo + NTLM.Flag56Bit;
            ...
            try {
                ...
                if (authSess.isGuest()) {
                    ...
                    if (allowGuest() == true) {
                        // Get a guest authentication token
                        doGuestLogon(client, sess);
                        LOGGER.debug("Passthru authenticate user={}, GUEST", userName);
                    }
                } else {
                    // Indicate that the client is logged on
                    client.setLogonType(ClientInfo.LogonNormal);
                    LOGGER.debug("Passthru authenticate user={}, FULL", userName);
                }
    ...
    private final byte[] doSpnegoSessionSetup(final SMBSrvSession sess, final ClientInfo client, final byte[] secbuf, final int secpos, final int seclen,
        ...
        if (tokType == SPNEGO.NegTokenTarg && sess.hasSetupObject(client.getProcessId())
            ...
            try {
                ...
                LOGGER.debug("Passthru error on session startup: {}", ex.getMessage());
        ...
        try {
            ...
            LOGGER.debug("Failed to encode NegTokenTarg: {}", ex.getMessage());
    ...
    public void initialize(final ServerConfiguration config, final ConfigElement params) throws InvalidConfigurationException {
        ...
        final ConfigElement protoOrder = params.getChild("protocolOrder");
        if (protoOrder != null) {
            // Parse the protocol order list
            final StringTokenizer tokens = new StringTokenizer(protoOrder.getValue(), ",");
            ...
            if (tokens.hasMoreTokens()) {
                ...
                final String primaryStr = tokens.nextToken();
                ...
                if (tokens.hasMoreTokens()) {
                    ...
                    final String secondaryStr = tokens.nextToken();
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Protocol order primary={}, secondary={}", Protocol.asString(primaryProto), Protocol.asString(secondaryProto));
            }
        }

        // Check if the offline check interval has been specified
        final ConfigElement checkInterval = params.getChild("offlineCheckInterval");
        if (checkInterval != null) {
            try {
                // Validate the check interval value
                final int offlineCheck = Integer.parseInt(checkInterval.getValue());
                ...
                LOGGER.debug("Using offline check interval of {} seconds", offlineCheck);
        ...
        final ConfigElement sessTmoElem = params.getChild("Timeout");
        if (sessTmoElem != null) {
            ...
            try {
                ...
                final int sessTmo = Integer.parseInt(sessTmoElem.getValue());

                // Range check the timeout

                if (sessTmo < MinSessionTmo || sessTmo > MaxSessionTmo) {
                    throw new InvalidConfigurationException("Invalid session timeout, valid range is " + MinSessionTmo + " to " + MaxSessionTmo);
                }
        ...
        String srvList = null;
        final ConfigElement srvNamesElem = params.getChild("Server");
        ...
        if (srvList != null) {
            ...
            final ConfigElement domNameElem = params.getChild("Domain");
        ...
        final SMBServer smbServer = (SMBServer) config.findServer("SMB");
        if (smbServer != null) {
     ...
     */
    @Override
    public void sessionClosed(final SrvSession sess) {
        ...
        if (passDetails != null) {
            ...
            try {
                ...
                LOGGER.debug("Passthru error closing session (closed) {}", ex.getMessage());
            }
        }
    }
     ...
     */
    @Override
    public void sessionCreated(final SrvSession sess) {
    }
     ...
     */
    @Override
    public void sessionLoggedOn(final SrvSession sess) {
        // Check if there is an active session to the authentication server for this local
        // session
        final PassthruDetails passDetails = m_sessions.get(sess.getUniqueId());
        if (passDetails != null) {
            ...
            try {
                ...
                LOGGER.debug("Passthru error closing session (logon) {}", ex.getMessage());
            }
        }
    }
}
