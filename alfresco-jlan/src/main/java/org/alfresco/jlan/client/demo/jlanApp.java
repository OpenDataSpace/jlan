...
public abstract class jlanApp {
  ...
  private Properties m_properties;
	...
	private final void loadProperties(PrintStream out)
		...
		if ( property != null && property.length() > 0) {
			...
			if ( resolve1.equals("NETBIOS") == false && resolve1.equals("DNS") == false) {
				...
				int resolveType = NetBIOSSession.WINSAndDNS;

				if ( resolve1.equals("NETBIOS") && resolve2 == null)
					resolveType = NetBIOSSession.WINSOnly;
				else if ( resolve1.equals("DNS") && resolve2 == null)
					resolveType = NetBIOSSession.DNSOnly;
		...
		if ( property != null && (property.equals("0") || property.equalsIgnoreCase("false"))) {
			...
			// Disable use of the *SMBSERVER wildcard name when connecting to a file server with
			// NetBIOS
	...
	protected final void setShare(PCShare shr) {
		...
		// Check if the user name and/or password should be set from the command line or properties
		// file
