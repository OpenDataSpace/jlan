    ...
    private void processNegotiateResponse() {
        ...
        if (m_pkt.getByteCount() > 0) {
            ...
            if (keyLen > 0) {
                ...
                if (Session.hasDebugOption(Session.DBGDumpPacket)) {
                    LOGGER.debug("** Encryption Key: {}, length = {}", HexDump.hexString(encryptKey), keyLen);
                }
            }
            ...
            if (Session.hasDebugOption(Session.DBGDumpPacket)) {
                LOGGER.info("** Server domain : {}.", getDomain());
            }
        }
    }
}
