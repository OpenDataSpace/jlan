...
public class TransPacket extends SMBPacket {
	...
	public final TransactBuffer doTransaction(Session sess, TransactBuffer tbuf)
		...
		try {
			...
			while ((paramBuf != null && paramBuf.getAvailableLength() > 0)
				...
				setCommand(tbuf.isType() == PacketType.Transaction ? PacketType.TransactionSecond : PacketType.Transaction2Second);
			...
			setCommand(tbuf.hasName() ? PacketType.Transaction : PacketType.Transaction2);
	...
	public final int getParameterBlockCount() {
		...
		else if ( isType() == PacketType.Transaction || isType() == PacketType.Transaction2)
	...
	public final int getDataBlockCount() {
		if ( isResponse())
			return getParameter(6);
		if ( isType() == PacketType.Transaction || isType() == PacketType.Transaction2)
