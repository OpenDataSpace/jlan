...
public class Session {
    ...
    /**
     * Allocate an SMB packet for this session. The preferred packet size is specified, if a smaller buffer size has been negotiated a smaller SMB packet will
     * be returned.
     *
     * @param pref
     *            Preferred SMB packet size
     ...
     *
     * @param cnt
     *            Number of packets to echo from the remote server
     * @exception java.io.IOException
     *                If an I/O error occurs
     * @exception SMBException
     *                SMB error occurred
     ...
     *
     * @param dbg
     *            Bit mask of debug options to enable, or zero to disable
     */
     ...
     *
     * @param dia
     *            SMB dialect that this session is using.
     */
     ...
     *
     * @param lm
     *            Servers LAN manager type string.
     */
     ...
     *
     * @param os
     *            Servers operating system type string.
     */
     ...
     *
     * @param netSess
     *            Network session that this SMB session is to be associated with.
     */
     ...
     *
     * @param id
     *            Tree identifier for this session.
     */
     ...
     *                If the MD5 message digest is not available
     */
    ...
    protected final void signTxPacket(SMBPacket pkt) {
        ...
        if (LOGGER.isInfoEnabled() && hasDebugOption(DBGSigning)) {
            LOGGER.info("Sign request {}, seq={}, signature = 0x{}", PacketType.getCommandName(pkt.getCommand()), seqNo, Long.toHexString(m_lastTxSig));
            LOGGER.info("Send length = {}", pkt.getLength());
        }
    }
     ...
     *            SMBPacket
     * @exception SMBException
     *                If the received packet SMB signature is not valid
     */
    protected final void verifyRxPacket(SMBPacket pkt) throws SMBException {
        ...
        if (hasActiveTransaction() == false) {
            ...
            if (pkt.isValidResponse()) {
                ...
                if (LOGGER.isInfoEnabled() && hasDebugOption(DBGSigning))
                    LOGGER.info("Transaction error returned, signature NOT checked");
        ...
        if (LOGGER.isInfoEnabled() && hasDebugOption(DBGSigning)) {
            LOGGER.info("Verify sign {}, seq={}, signature = 0x{}, calc={}", PacketType.getCommandName(pkt.getCommand()), seqNo, Long.toHexString(rxSig),
                    Long.toHexString(calcSig));
            LOGGER.info("Received length = {}", pkt.getLength());
            if (calcSig != rxSig) {
                LOGGER.info("#### SMB signature mismatch ####");
            }
        }
        ...
        if (calcSig != rxSig) {
            ...
            if (rxSig == m_lastTxSig) {
                ...
                if (hasDebugOption(DBGSigning)) {
                    LOGGER.info("Received signature equals sent, disabling signing");
                }
            } else {
                // Bad signature received
                throw new SMBException(SMBStatus.JLANErr, SMBStatus.JLANInvalidSMBSignature);
            }
        }
    }
