...
public class DBConnectionPool {
            ...
            while (m_shutdown == false) {
                ...
                synchronized (m_allocPool) {
                    ...
                    while (enm.hasMoreElements()) {
                        ...
                        if (expire.longValue() != PermanentLease && expire.longValue() < timeNow) {
                            ...
                            if (removeConn == true) {
                                LOGGER.warn("DBConnectionReaper Permanent lease connection error");
                            }
                        }
                        ...
                        if (removeConn == true) {
                            ...
                            try {
                                conn.close();
                                LOGGER.warn("DBConnectionReaper closed expired connection, conn={}", conn);
                            } catch (SQLException ex) {
                            }
                        }
                    }
                ...
                synchronized (m_freePool) {
                    ...
                    while (m_freePool.size() > getMaximumPoolSize()) {
                        LOGGER.debug("DBConnectionReaper trimming free pool, {}/{}", m_freePool.size(), getMaximumPoolSize());
    ...
    /**
     * Get a connection from the pool with the specified lease time. A lease time of -1 indicates that the connection lease is permanent.
     *
     * @param expireTime
     *            long
     * @return Connection
        ...
        synchronized (m_freePool) {
            ...
            if (m_freePool.size() > 0) {
                ...
                try {
                    ...
                    conn = null;
                    LOGGER.warn("%%%%% SQL Connection Error", ex);
                }
    ...
    protected final void notifyConnectionPoolState() {
        LOGGER.debug("DBConnectionPool: Database server is " + (isOnline() ? "OnLine" : "OffLine"));
        // Inform the connection pool listener
        if (hasConnectionPoolListener()) {
            m_dbListener.databaseOnlineStatus(isOnline());
        } else {
            LOGGER.debug("DBConnectionPool: No listener");
        }
    }
}
