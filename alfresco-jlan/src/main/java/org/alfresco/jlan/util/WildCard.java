...
public final class WildCard {
     ...
     *            boolean
     */
    public WildCard(final String pattern, final boolean caseSensitive) {
     ...
     * @return boolean
     */
    public final boolean matchesPattern(final String str) {
     ...
     * @return boolean
     */
    protected final boolean matchComplexWildcard(final String str) {
        ...
        int wpos = 0;
        final int wlen = m_patternChars.length;

        int spos = 0;
        final int slen = strChars.length;
     ...
     *            boolean
     */
    public final void setPattern(final String pattern, final boolean caseSensitive) {
     ...
     *            int
     */
    private final void setType(final int typ) {
        m_type = typ;
    }
     ...
     *            java.lang.String
     */
    public final static boolean containsWildcards(final String str) {
     ...
     * @return boolean
     */
    public final static boolean containsUnicodeWildcard(final String str) {

        // Check if the string contains any of the Unicode wildcards

        if (str.indexOf(SINGLECHAR_UNICODE_WILDCARD) != -1 || str.indexOf(MULTICHAR_UNICODE_WILDCARD) != -1 || str.indexOf(DOT_UNICODE_WILDCARD) != -1) {
            return true;
        }
        return false;
    }
     ...
     * @return String
     */
    public final static String convertUnicodeWildcardToDOS(final String str) {

        // Create a buffer for the new wildcard string

        final StringBuffer newStr = new StringBuffer(str.length());
     ...
     * @return String
     */
    public final static String convertToRegexp(final String path) {

        // Convert the path to characters, check if the wildcard string ends with a single character
        // wildcard

        final char[] smbPattern = path.toCharArray();
        final boolean endsWithQ = smbPattern[smbPattern.length - 1] == '?';
     ...
     * @return boolean
     */
    public static final boolean isWildcardAll(final String srchPath) {
     ...
     * 
     * WildCard wcard = new WildCard(wildcards[i], false); out.println(wcard + ":");
     * 
     * // Test the file names
     * 
     * for ( int j = 0; j < names.length; j++) { out.print("  "); out.print(names[j]); out.print(": "); out.println(wcard.matchesPattern(names[j]) ? "Match" :
     * "NoMatch"); } out.println();
     * 
     * // Create a wildcard, case sensitive check
     * 
     * wcard = new WildCard(wildcards[i], true); out.println(wcard + ":");
     * 
     * // Test the file names
     * 
     * for ( int j = 0; j < names.length; j++) { out.print("  "); out.print(names[j]); out.print(": "); out.println(wcard.matchesPattern(names[j]) ? "Match" :
     * "NoMatch"); } out.println("----------------------------------------"); } } catch ( Exception ex) { out.println("Error: " + ex.toString()); } }
     **/
}
