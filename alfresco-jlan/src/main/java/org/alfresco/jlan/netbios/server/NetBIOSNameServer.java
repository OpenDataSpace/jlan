...
public class NetBIOSNameServer extends NetworkServer implements Runnable, ConfigurationListener {
    private static final Logger LOGGER = LoggerFactory.getLogger(NetBIOSNameServer.class);
    ...
    class NetBIOSRequestHandler extends Thread {
            ...
            while (m_hshutdown == false) {
                try {
                    ...
                    synchronized (m_reqList) {
                        ...
                        if (m_reqList.size() == 0) {
                            LOGGER.info("NetBIOS handler waiting for request ...");
                    ...
                    while (req.hasErrorStatus() == false && retry++ < reqRetry) {
                        LOGGER.info("NetBIOS handler, processing " + req);
                    ...
                    if (req.hasErrorStatus() == false) {
                        LOGGER.info("NetBIOS handler successful, ()", req);

                        // Update the name record
                        final NetBIOSName nbName = req.getNetBIOSName();
         ...
         * @return boolean
         */
        private final boolean sendAddName(final NetBIOSRequest req, final InetAddress dest, final boolean bcast) {
            try {
                // Allocate a buffer for the add name NetBIOS packet
                final byte[] buf = new byte[AddNameSize];
                final NetBIOSPacket addPkt = new NetBIOSPacket(buf);
                ...
                for (int i = 0; i < req.getNetBIOSName().numberOfAddresses(); i++) {
                    ...
                    final int len = addPkt.buildAddNameRequest(req.getNetBIOSName(), i, req.getTransactionId());
                    if (bcast == false) {
                        addPkt.setFlags(0);
                    }

                    // Allocate the datagram packet, using the add name buffer
                    final DatagramPacket pkt = new DatagramPacket(buf, len, dest, getPort());
                    ...
                    LOGGER.info("  Add name {}, {}", (bcast ? "broadcast" : "WINS"), req);
                }
         ...
         * @return boolean
         */
        private final boolean sendRefreshName(final NetBIOSRequest req, final InetAddress dest, final boolean bcast) {
            try {
                // Allocate a buffer for the refresh name NetBIOS packet
                final byte[] buf = new byte[RefreshNameSize];
                final NetBIOSPacket refreshPkt = new NetBIOSPacket(buf);

                // Build a refresh name packet for each IP address
                for (int i = 0; i < req.getNetBIOSName().numberOfAddresses(); i++) {
                    // Build a refresh name request for the current IP address
                    final int len = refreshPkt.buildRefreshNameRequest(req.getNetBIOSName(), i, req.getTransactionId());
                    if (bcast == false) {
                    ...
                    LOGGER.info("  Refresh name {}, {}", (bcast ? "broadcast" : "WINS"), req);
                }
            } catch (final IOException ex) {
                req.setErrorStatus(true);
                return false;
         ...
         * @return boolean
         */
        private final boolean sendDeleteName(final NetBIOSRequest req, final InetAddress dest, final boolean bcast) {
            try {
                // Allocate a buffer for the delete name NetBIOS packet
                final byte[] buf = new byte[DeleteNameSize];
                final NetBIOSPacket delPkt = new NetBIOSPacket(buf);

                // Build a delete name packet for each IP address
                for (int i = 0; i < req.getNetBIOSName().numberOfAddresses(); i++) {
                    // Build an add name request for the current IP address
                    final int len = delPkt.buildDeleteNameRequest(req.getNetBIOSName(), i, req.getTransactionId());
                    if (bcast == false) {
                        delPkt.setFlags(0);
                    }
                    ...
                    LOGGER.info("  Delete name {}, {}", (bcast ? "broadcast" : "WINS"), req);
                }
            } catch (final IOException ex) {
                req.setErrorStatus(true);
                return false;
        ...
        public void run() {
            ...
            while (m_hshutdown == false) {
                try {
                    ...
                    LOGGER.info("NetBIOS name refresh wakeup ...");
                    ...
                    synchronized (m_localNames) {
                        ...
                        final long expireTime = System.currentTimeMillis() + NameRefreshWakeupInterval;

                        // Loop through the local name list
                        for (int i = 0; i < m_localNames.size(); i++) {
                            // Get a name from the list
                            final NetBIOSName nbName = m_localNames.elementAt(i);

                            // Check if the name has expired, or will expire before the next wakeup event
                            if (nbName.getExpiryTime() < expireTime) {
                                LOGGER.info("Queuing name refresh for {}", nbName);

                                // Queue a refresh request for the NetBIOS name
                                final NetBIOSRequest nbReq = new NetBIOSRequest(NetBIOSRequest.RefreshName, nbName, getNextTransactionId());
     ...
     *                If a network setup error occurs
     */
    public NetBIOSNameServer(final ServerConfiguration config) throws SocketException {
     ...
     *
     * @param name
     *            NetBIOS name to be added
     * @exception java.io.IOException
     *                I/O error occurred.
     */
    public final synchronized void AddName(final NetBIOSName name) throws IOException {
        ...
        final NetBIOSRequest nbReq = new NetBIOSRequest(NetBIOSRequest.AddName, name, getNextTransactionId());
     ...
     *
     * @param name
     *            NetBIOS name to be deleted
     * @exception java.io.IOException
     *                I/O error occurred.
     */
    public final synchronized void DeleteName(final NetBIOSName name) throws IOException {
        // Check if the NetBIOS name socket has been initialized
        if (m_socket == null) {
            throw new IOException("NetBIOS name socket not initialized");
        }
        ...
        final NetBIOSRequest nbReq = new NetBIOSRequest(NetBIOSRequest.DeleteName, name, getNextTransactionId(), DeleteNameRetries);
     ...
     *            AddNameListener
     */
    public final synchronized void addAddNameListener(final AddNameListener l) {
     ...
     *            QueryNameListener
     */
    public final synchronized void addQueryListener(final QueryNameListener l) {
     ...
     *            RemoteNameListener
     */
    public final synchronized void addRemoteListener(final RemoteNameListener l) {
     ...
     *            int
     */
    protected final synchronized void fireAddNameEvent(final NetBIOSName name, final int sts) {
        ...
        final NetBIOSNameEvent evt = new NetBIOSNameEvent(name, sts);

        // Inform all registered listeners
        for (int i = 0; i < m_addListeners.size(); i++) {
            final AddNameListener addListener = m_addListeners.elementAt(i);
     ...
     *            InetAddress
     */
    protected final synchronized void fireQueryNameEvent(final NetBIOSName name, final InetAddress addr) {
        // Check if there are any listeners
        ...
        // Create a NetBIOS name event
        final NetBIOSNameEvent evt = new NetBIOSNameEvent(name, NetBIOSNameEvent.QUERY_NAME);

        // Inform all registered listeners
        for (int i = 0; i < m_queryListeners.size(); i++) {
            final QueryNameListener queryListener = m_queryListeners.elementAt(i);
     ...
     *            InetAddress
     */
    protected final synchronized void fireNameRegisterEvent(final NetBIOSName name, final InetAddress addr) {
        // Check if there are any listeners
        ...
        // Create a NetBIOS name event
        final NetBIOSNameEvent evt = new NetBIOSNameEvent(name, NetBIOSNameEvent.REGISTER_NAME);

        // Inform all registered listeners
        for (int i = 0; i < m_remoteListeners.size(); i++) {
            final RemoteNameListener nameListener = m_remoteListeners.elementAt(i);
     ...
     *            InetAddress
     */
    protected final synchronized void fireNameReleaseEvent(final NetBIOSName name, final InetAddress addr) {
        // Check if there are any listeners
        if (m_remoteListeners == null || m_remoteListeners.size() == 0) {
            return;
        }
     ...
     *            int
     */
    protected final void processNameQuery(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
        ...
        final char nameType = searchName.charAt(15);
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("%% Query name={}, type={}, len={}", searchName, NetBIOSName.TypeAsString(nameType), len);
        }

        // Search for the name in the local name table
        final Enumeration<NetBIOSName> enm = m_localNames.elements();
        ...
        while (enm.hasMoreElements() && foundName == false) {
            ...
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("NetBIOS Name - {}, len={}, type={}", nbName.getName(), nbName.getName().length(), NetBIOSName.TypeAsString(nbName.getType()));
            }
        ...
        if (foundName == true) {
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("%% Found name " + searchName + " in local name table : " + nbName.toString());
            }

            // Build the name query response
            final int pktLen = pkt.buildNameQueryResponse(nbName);

            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("%% NetBIOS Reply to " + fromAddr.getHostAddress() + " :-");
            ...
            LOGGER.info("%% Failed to find match for name " + searchName);
        }
    }
     ...
     *            int
     */
    protected final void processNameRegister(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
        ...
        final char nameType = regName.charAt(15);
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("%% Register name=" + regName + ", type=" + NetBIOSName.TypeAsString(nameType) + ", len=" + len);
        }

        // Create a NetBIOS name for the host
        final byte[] hostIP = fromAddr.getAddress();
        final NetBIOSName nbName = new NetBIOSName(regName, nameType, false, hostIP);
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("%% Added remote name " + nbName.toString() + " to remote names table");
        }
    }
     ...
     *            int
     */
    protected final void processNameRelease(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("%% Release name=" + regName + ", type=" + NetBIOSName.TypeAsString(nameType) + ", len=" + len);
        }
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("%% Released remote name " + nbName.toString() + " from remote names table");
        }
    }
     ...
     *            int
     */
    protected final void processQueryResponse(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
    }
     ...
     *            int
     */
    protected final void processRegisterResponse(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
        ...
        final int tranId = pkt.getTransactionId();

        // Find the matching request
        final NetBIOSRequest req = findRequest(tranId);
        if (req == null) {
            return;
        }

        // Get the error code from the response
        final int errCode = pkt.getResultCode();
        ...
        if (errCode != 0) {
            ...
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("%% Negative Name Registration name=" + nbName);
            }
            ...
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("%% Name Registration Successful name=" + req.getNetBIOSName().getName());
            }
     ...
     *            int
     */
    protected final void processReleaseResponse(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
    }
     ...
     *            int
     */
    protected final void processWack(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
    }
     ...
     *            int
     */
    protected final void processAdapterStatus(final NetBIOSPacket pkt, final InetAddress fromAddr, final int fromPort) {
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("%% Adapter status request");
        }

        // Build the local name list
        final NetBIOSNameList nameList = new NetBIOSNameList();
        for (int i = 0; i < m_localNames.size(); i++) {
            nameList.addName(m_localNames.get(i));
        }

        // Build the name query response
        final int pktLen = pkt.buildAdapterStatusResponse(nameList, hasPrimaryWINSServer() ? NetBIOSPacket.NAME_TYPE_PNODE : NetBIOSPacket.NAME_TYPE_BNODE);
     ...
     *            AddNameListener
     */
    public final synchronized void removeAddNameListener(final AddNameListener l) {
    ...
    /**
     * Remove a query name listener from the NetBIOS name server.
     *
     * @param l
     *            QueryNameListener
     */
    public final synchronized void removeQueryNameListener(final QueryNameListener l) {
        // Check if the listener list is valid
        if (m_queryListeners == null) {
            return;
        }
     ...
     *            RemoteNameListener
     */
    public final synchronized void removeRemoteListener(final RemoteNameListener l) {
        // Check if the listener list is valid
        if (m_remoteListeners == null) {
            return;
        }
    ...
    public void run() {
        ...
        try {
            ...
            final Vector ipList = new Vector();
            if (hasBindAddress()) {
                // Use the specified bind address
                ipList.addElement(getBindAddress().getAddress());
            } else {
                // Get a list of all the local addresses
                final InetAddress[] addrs = InetAddress.getAllByName(InetAddress.getLocalHost().getHostName());

                for (final InetAddress addr : addrs) {
                    // Check for a valid address, filter out '127.0.0.1' and '0.0.0.0' addresses
                    if (addr.getHostAddress().equals("127.0.0.1") == false && addr.getHostAddress().equals("0.0.0.0") == false) {
                        ipList.addElement(addr.getAddress());
                    }
                }
                ...
                if (ipList.size() == 0) {
                    ...
                    final Enumeration niEnum = NetworkInterface.getNetworkInterfaces();

                    if (niEnum != null) {
                        while (niEnum.hasMoreElements()) {
                            // Get the current network interface
                            final NetworkInterface ni = (NetworkInterface) niEnum.nextElement();

                            // Enumerate the addresses for the network adapter
                            final Enumeration niAddrs = ni.getInetAddresses();
                            if (niAddrs != null) {
                                // Check for any valid addresses
                                while (niAddrs.hasMoreElements()) {
                                    final InetAddress curAddr = (InetAddress) niAddrs.nextElement();

                                    if (curAddr.getHostAddress().equals("127.0.0.1") == false && curAddr.getHostAddress().equals("0.0.0.0") == false) {
                        ...
                        if (LOGGER.isInfoEnabled() && ipList.size() > 0) {
                            LOGGER.info("Found " + ipList.size() + " addresses using interface list");
                        }
                    }
                }
            }
            ...
            if (getCIFSConfiguration().hasAliasNames()) {
                ...
                final StringList names = getCIFSConfiguration().getAliasNames();

                for (int i = 0; i < names.numberOfStrings(); i++) {
                    // Get the current alias name
                    final String alias = names.getStringAt(i);
        ...
        if (hasException() == false) {
            ...
            while (m_shutdown == false) {
                try {
                    ...
                    final InetAddress fromAddr = pkt.getAddress();
                    final int fromPort = pkt.getPort();
                    ...
                    switch (nbPkt.getOpcode()) {
                        ...
                        default:
                            if (LOGGER.isErrorEnabled()) {
                                final int opCode = nbPkt.getOpcode();
                                LOGGER.error("Unknown OpCode 0x" + Integer.toHexString(opCode) + ", " + opCode);
                            }
                            break;
                    }
                } catch (final Exception ex) {
     ...
     */
    protected final void sendPacket(final NetBIOSPacket nbpkt, final int len) throws java.io.IOException {
        // Allocate the datagram packet, using the add name buffer
        final DatagramPacket pkt = new DatagramPacket(nbpkt.getBuffer(), len, NetworkSettings.getBroadcastAddress(), getPort());
     ...
     *                The exception description.
     */
    protected final void sendPacket(final NetBIOSPacket nbpkt, final int len, final InetAddress replyAddr, final int replyPort) throws java.io.IOException {
        // Allocate the datagram packet, using the add name buffer
        final DatagramPacket pkt = new DatagramPacket(nbpkt.getBuffer(), len, replyAddr, replyPort);
     ...
     */
    public final void setBindAddress(final InetAddress addr) {
        m_bindAddress = addr;
    }
     ...
     *            InetAddress
     */
    public final void setPrimaryWINSServer(final InetAddress addr) {
        m_winsPrimary = addr;
    }
     ...
     *            InetAddress
     */
    public final void setSecondaryWINSServer(final InetAddress addr) {
     ...
     */
    private final NetBIOSRequest findRequest(final int id) {
        ...
        synchronized (m_reqList) {
            ...
            while (req == null && idx < m_reqList.size()) {
                ...
                final NetBIOSRequest curReq = m_reqList.elementAt(idx++);
    ...
    /**
     * Start the NetBIOS name server is a separate thread
     */
    @Override
    public void startServer() {
        // Create a separate thread to run the NetBIOS name server
        ...
        try {
            ...
            switch (id) {
                ...
                case ConfigId.SMBNetBIOSEnable:
                    ...
                    final Boolean enaSMB = (Boolean) newVal;
                ...
                case ConfigId.NetBIOSDebugEnable:
                    ...
                    final Boolean b = (Boolean) newVal;
