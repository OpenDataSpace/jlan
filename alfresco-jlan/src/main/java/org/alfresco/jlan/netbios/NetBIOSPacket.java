 ...
 *
 * <p>
 * A NetBIOSPacket holds a single NetBIOS request or response. Contains methods for building the main NetBIOS request types (add name, delete name, name query,
 * refresh and session setup).
 *
 * @author gkspencer
 */
public class NetBIOSPacket {
    ...
    // NetBIOS name registration error response codes (RCODE field)
     ...
     *            byte[]
     */
    public NetBIOSPacket(final byte[] buf) {
        m_nbbuf = buf;
        m_datalen = NB_DATA;
    }
     ...
     *            int
     */
    public NetBIOSPacket(final int siz) {
        m_nbbuf = new byte[siz];
        m_datalen = NB_DATA;
    }

    /**
     * Dump the packet structure to string.
     *
     * @param sessPkt
     *            True if this is a NetBIOS session packet, else false.
     */
    public String DumpPacket(final boolean sessPkt) {
        final String ln = System.lineSeparator();
        final StringBuilder stringBuilder = new StringBuilder();
        // Display the transaction id
        stringBuilder.append("NetBIOS Packet Dump :-").append(ln);
        ...
        if (sessPkt == true) {
            switch (getPacketType()) {
                ...
                case RFCNetBIOSProtocol.SESSION_REQUEST:
                    ...
                    stringBuilder.append("Session request from " + NetBIOSSession.DecodeName(name.toString())).append(ln);
                    break;
            ...
            stringBuilder.append(" Transaction Id : " + getTransactionId()).append(ln);
            ...
            switch (getOpcode()) {
                case NAME_QUERY:
                    ...
                    break;
                case RESP_QUERY:
                    stringBuilder.append("QUERY (Response)");
                    break;
                case NAME_REGISTER:
                    stringBuilder.append("NAME REGISTER");
                    break;
                case RESP_REGISTER:
                    stringBuilder.append("NAME REGISTER (Response)");
                    break;
                case NAME_RELEASE:
                    stringBuilder.append("NAME RELEASE");
                    break;
                case RESP_RELEASE:
                    stringBuilder.append("NAME RELEASE (Response)");
                    break;
                case WACK:
                    stringBuilder.append("WACK");
                    break;
                case REFRESH:
                    stringBuilder.append("REFRESH");
                    break;
                default:
                    stringBuilder.append(Integer.toHexString(getOpcode()));
                    break;
            }
            stringBuilder.append(ln);
            // Display the flags
            stringBuilder.append(" Flags  : " + Integer.toHexString(getFlags())).append(ln);

            // Display the name counts
            stringBuilder.append(" QDCount : " + getQuestionCount()).append(ln);
            stringBuilder.append(" ANCount : " + getAnswerCount()).append(ln);
            stringBuilder.append(" NSCount : " + getNameServiceCount()).append(ln);
            stringBuilder.append(" ARCount : " + getAdditionalCount()).append(ln);
            ...
            if (getQuestionCount() > 0) {
                ...
                final StringBuffer encName = new StringBuffer();
                for (int i = 1; i <= 32; i++) {
                    encName.append((char) m_nbbuf[NB_DATA + i]);
                }

                // Decode the name
                final String name = NetBIOSSession.DecodeName(encName.toString());
                stringBuilder.append(name + " <" + NetBIOSName.TypeAsString(name.charAt(15)) + ">").append(ln);
            }
        }

        // Dump the raw data
        stringBuilder.append("********** Raw NetBIOS Data Dump **********").append(ln);
        final ByteArrayOutputStream baos = new ByteArrayOutputStream();
        final PrintStream ps = new PrintStream(baos);
        HexDump.Dump(getBuffer(), getLength(), 0, ps);
    ...
    public final NetBIOSNameList getAnswerNameList() {
        ...
        while (cnt-- > 0) {
            ...
            final int nameLen = NetBIOSName.decodeNetBIOSNameLength(m_nbbuf, pos);
            final NetBIOSName name = NetBIOSName.decodeNetBIOSName(m_nbbuf, pos);
            ...
            while (dataCnt > 0) {
                ...
                final int flags = DataPacker.getShort(m_nbbuf, pos);
                pos += 2;
    ...
    public final NetBIOSNameList getAdapterStatusNameList() {
        ...
        final int nameLen = m_nbbuf[pos++] & 0xFF;
        pos += nameLen;
        ...
        while (nameCnt > 0 && dataCnt > 0) {
            ...
            final NetBIOSName nbName = new NetBIOSName(m_nbbuf, pos);
            pos += 16;

            // Get the name type flags, check if this is a unique or group name
            final int typ = DataPacker.getShort(m_nbbuf, pos);
            pos += 2;
     ...
     *            int
     */
    private void setMACAddress(final NetBIOSNameList nameList, final int pos) {
    ...
    public final int getResultCode() {
        final int res = DataPacker.getShort(m_nbbuf, NB_OPCODE) & MASK_RCODE;
        return res;
    }
     ...
     *            int
     */
    public final void setAdditionalCount(final int cnt) {
     ...
     *            int
     */
    public final void setAnswerCount(final int cnt) {
     ...
     *            int
     */
    public final int setAnswerName(final String name, final char ntyp, final int qtyp, final int qcls) {
        // RFC encode the NetBIOS name string
        final String encName = NetBIOSSession.ConvertName(name, ntyp);
        final byte[] nameByts = encName.getBytes();
     ...
     *            int
     */
    public final void setFlags(final int flg) {
     ...
     *            int
     */
    public final void setHeaderFlags(final int flg) {
     ...
     *            int
     */
    public final void setHeaderLength(final int len) {
     ...
     *            int
     */
    public final void setHeaderType(final int typ) {
     ...
     *            byte[]
     */
    public final int setIPAddress(final int off, final byte[] ipaddr) {
        // Pack the IP address
        for (int i = 0; i < 4; i++) {
     ...
     *            int
     */
    public final int setNameRegistrationFlags(final int off, final int flg) {
        // Set the name registration flags
     ...
     *            int
     */
    public final void setNameServiceCount(final int cnt) {
     ...
     *            int
     */
    public final void setOpcode(final int op) {
     ...
     *            int
     */
    public final void setQuestionCount(final int cnt) {
     ...
     * @return int
     */
    public final int setQuestionName(final NetBIOSName name, final int qtyp, final int qcls) {
        // Encode the NetBIOS name
        final byte[] nameByts = name.encodeName();
     ...
     *            int
     */
    public final int setQuestionName(final String name, final char ntyp, final int qtyp, final int qcls) {
        // RFC encode the NetBIOS name string
        final String encName = NetBIOSSession.ConvertName(name, ntyp);
        final byte[] nameByts = encName.getBytes();
     ...
     *            int
     */
    public final int setResourceData(final int off, final int flg, final byte[] data, final int len) {
     ...
     *            int
     */
    public final int setResourceDataLength(final int off, final int len) {
     ...
     *            int
     */
    public final int setResourceRecord(final int pktoff, final int rroff, final int qtyp, final int qcls) {
     ...
     *            int
     */
    public final void setTransactionId(final int id) {
     ...
     *            int
     */
    public final int setTTL(final int off, final int ttl) {
     ...
     * @return String
     */
    public final static String getTypeAsString(final int typ) {
     ...
     * @return int
     */
    public final int buildNameQueryResponse(final NetBIOSName name) {
        // Fill in the header
        ...
        for (int i = 0; i < name.numberOfAddresses(); i++) {
            ...
            final byte[] ipaddr = name.getIPAddress(i);
     ...
     * @return int
     */
    public final int buildAddNameRequest(final NetBIOSName name, final int addrIdx, final int tranId) {
     ...
     * @return int
     */
    public final int buildRefreshNameRequest(final NetBIOSName name, final int addrIdx, final int tranId) {
        // Initialize an add name NetBIOS packet
        setTransactionId(tranId);
     ...
     * @return int
     */
    public final int buildDeleteNameRequest(final NetBIOSName name, final int addrIdx, final int tranId) {
     ...
     * @return int
     */
    public final int buildNameQueryRequest(final NetBIOSName name, final int tranId, final boolean wins) {
     ...
     * @return int
     */
    public final int buildSessionSetupRequest(final NetBIOSName fromName, final NetBIOSName toName) {
     ...
     * @return int
     */
    public final int buildAdapterStatusResponse(final NetBIOSNameList nameList, final int nodeType) {
        // Fill in the header
        setOpcode(NetBIOSPacket.RESP_QUERY);
        setFlags(NetBIOSPacket.FLG_RECURSDES + NetBIOSPacket.FLG_AUTHANSWER);
