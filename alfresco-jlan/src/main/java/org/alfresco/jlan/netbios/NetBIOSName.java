...
public class NetBIOSName {
    private static final Logger LOGGER = LoggerFactory.getLogger(NetBIOSName.class);
    // NetBIOS name length
     ...
     *            boolean
     */
    public NetBIOSName(final String name, final char typ, final boolean group) {
        setName(name);
        setType(typ);
        setGroup(group);
    }
     ...
     *            NetBIOSName
     */
    public NetBIOSName(final NetBIOSName nbName) {
     ...
     *            byte[]
     */
    public NetBIOSName(final String name, final char typ, final boolean group, final byte[] ipaddr) {
        setName(name);
        setType(typ);
        setGroup(group);
        addIPAddress(ipaddr);
     ...
     *            Vector<byte[]>
     */
    public NetBIOSName(final String name, final char typ, final boolean group, final Vector<byte[]> ipList) {
        setName(name);
        setType(typ);
        setGroup(group);
     ...
     *            int
     */
    public NetBIOSName(final String name, final char typ, final boolean group, final byte[] ipaddr, final int ttl) {
        setName(name);
        setType(typ);
        setGroup(group);
        addIPAddress(ipaddr);
     ...
     *            int
     */
    public NetBIOSName(final String name, final char typ, final boolean group, final Vector<byte[]> ipList, final int ttl) {
        setName(name);
        setType(typ);
        setGroup(group);
        addIPAddresses(ipList);
     ...
     *            int
     */
    public NetBIOSName(final byte[] buf, final int off) {
     ...
     *            String
     */
    public NetBIOSName(final String name) {
        final int l = name.length() > NameLength ? NameLength : name.length();
     ...
     *            String
     */
    protected NetBIOSName(final String name, final String scope) {
        final int l = name.length() > NameLength ? NameLength : name.length();
        setName(name.substring(0, l - 1).trim());
        setType(name.charAt(NameLength - 1));
    ...
    public boolean equals(final Object obj) {
        ...
        if (obj instanceof NetBIOSName) {
            ...
            final NetBIOSName nbn = (NetBIOSName) obj;
     ...
     * @return byte[]
     */
    public final byte[] getIPAddress(final int idx) {
     ...
     * @return String
     */
    public final String getIPAddressString(final int idx) {
        if (m_addrList == null || idx < 0 || idx >= m_addrList.size()) {
            return null;
        }
     ...
     *            long
     */
    public final void setExpiryTime(final long expires) {
        m_expiry = expires;
    }
     ...
     *            int
     */
    public final void setTimeToLive(final int ttl) {
        m_ttl = ttl;
    }
     ...
     *            boolean
     */
    public final void setGroup(final boolean flag) {
        m_group = flag;
    }
     ...
     *            String
     */
    public final void setNameScope(final String scope) {
        if (scope == null) {
            m_scope = null;
     ...
     *            byte[]
     */
    public final void addIPAddress(final byte[] ipaddr) {
        if (m_addrList == null) {
     ...
     *            Vector (of byte[])
     */
    public final void addIPAddresses(final Vector<byte[]> addrList) {
        if (m_addrList == null) {
            m_addrList = new Vector<byte[]>();
        }
        ...
        for (int i = 0; i < addrList.size(); i++) {
            final byte[] addr = addrList.elementAt(i);
            m_addrList.add(addr);
        }
    }
     ...
     *            boolean
     */
    public final void setLocalName(final boolean local) {
        m_local = local;
    }
     ...
     *            char
     */
    public final void setType(final char typ) {
        m_type = typ;
    }
     ...
     *            int
     */
    public final void setNameNumber(final int nameNum) {
        m_nameNum = nameNum;
    }
    ...
    public static String toUpperCaseName(String name) {
        ...
        if (name != null && name.length() > 0) {
            final StringBuffer upperName = new StringBuffer(name.length());
     ...
     *            String
     */
    public final static void setNameConversionCharacterSet(final String charSet) {
    ...
    public byte[] encodeName() {
        ...
        final StringBuffer nbName = new StringBuffer(getName().toUpperCase());
        ...
        final byte[] encBuf = new byte[len];
        ...
        while (idx < nbName.length()) {
            ...
            final char ch = nbName.charAt(idx++);
        ...
        if (hasNameScope()) {
            ...
            final StringTokenizer tokens = new StringTokenizer(getNameScope(), ".");
    ...
    /**
     * Find the best match address that the NetBIOS name is registered on that matches one of the local TCP/IP addresses
     *
     * @param addrList
     *            InetAddress[]
     * @return int
     */
    public final int findBestMatchAddress(final InetAddress[] addrList) {
        ...
        for (final InetAddress element : addrList) {
            // Get the address bytes for the current local address
            final byte[] localAddr = element.getAddress();
            ...
            for (int addrIdx = 0; addrIdx < numberOfAddresses(); addrIdx++) {
                ...
                final byte[] remoteAddr = m_addrList.elementAt(addrIdx);
                int ipIdx = 0;
     ...
     * @return NetBIOSName
     */
    public static NetBIOSName decodeNetBIOSName(final byte[] buf, int off) {
        // Convert the RFC NetBIOS name string to a normal NetBIOS name string
        final StringBuffer nameBuf = new StringBuffer(NameLength);
        ...
        final StringBuffer scopeBuf = new StringBuffer(128);
        nameLen = buf[off++];
     ...
     * @return int
     */
    public static int decodeNetBIOSNameLength(final byte[] buf, int off) {
     ...
     * @return String
     */
    public final static String TypeAsString(final char typ) {
