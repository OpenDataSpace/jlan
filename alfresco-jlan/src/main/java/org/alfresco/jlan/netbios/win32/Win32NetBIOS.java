...
public class Win32NetBIOS {
    private static final Logger LOGGER = LoggerFactory.getLogger(Win32NetBIOS.class);
    // Constants
    //
     ...
     * @return int
     */
    public static int FindName(final int lana, final NetBIOSName nbName) {
        // Allocate a buffer to receive the name details
        final byte[] nameBuf = new byte[nbName.isGroupName() ? 65535 : 4096];

        // Get the raw NetBIOS name data
        final int sts = FindNameRaw(lana, nbName.getNetBIOSName(), nameBuf, nameBuf.length);
        ...
        final DataBuffer buf = new DataBuffer(nameBuf, 0, nameBuf.length);

        final int nodeCount = buf.getShort();
        buf.skipBytes(1);

        final boolean isGroupName = buf.getByte() == 0 ? false : true;
    ...
    /**
     * Return a comma delimeted list of WINS server TCP/IP addresses, or null if no WINS servers are configured.
     *
     * @return String
     */
     ...
     * @return String
     */
    public static final String getIPAddressForLANA(final int lana) {
        // Get the local NetBIOS name
        final String localName = GetLocalNetBIOSName();
        if (localName == null) {
            return null;
        }

        // Create a NetBIOS name for the local name
        final NetBIOSName nbName = new NetBIOSName(localName, NetBIOSName.WorkStation, false);
     ...
     * @return String
     */
    public static final String getAdapterNameForLANA(final int lana) {
        // Get the TCP/IP address for a LANA
        final String ipAddr = getIPAddressForLANA(lana);
        if (ipAddr == null) {
            return null;
        }

        // Get the list of available network adapters
        final Hashtable<String, NetworkInterface> adapters = getNetworkAdapterList();
        String adapterName = null;

        if (adapters != null) {
            // Find the network adapter for the TCP/IP address
            final NetworkInterface ni = adapters.get(ipAddr);
            if (ni != null) {
     ...
     * @return int
     */
    public static final int getLANAForIPAddress(final String addr) {
     ...
     * @return int
     */
    public static final int getLANAForAdapterName(final String name) {
        // Get the list of available network adapters
        final Hashtable<String, NetworkInterface> niList = getNetworkAdapterList();

        // Search for the address of the specified network adapter
        final Enumeration<String> niEnum = niList.keys();

        while (niEnum.hasMoreElements()) {
            // Get the current TCP/IP address
            final String ipAddr = niEnum.nextElement();
            final NetworkInterface ni = niList.get(ipAddr);
    ...
    private static final Hashtable<String, NetworkInterface> getNetworkAdapterList() {
        ...
        final Hashtable<String, NetworkInterface> niList = new Hashtable<String, NetworkInterface>();
        try {
            ...
            while (niEnum.hasMoreElements()) {
                ...
                final Enumeration<InetAddress> addrEnum = ni.getInetAddresses();
                while (addrEnum.hasMoreElements()) {
                    // Get the address and add the adapter to the list indexed via the numeric IP
                    // address string
                    final InetAddress addr = addrEnum.nextElement();
     ...
     *                If a Winsock error occurs
     */
    protected static native int CreateSocket(int lana) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static int BindSocket(final int sockPtr, final byte[] name) throws WinsockNetBIOSException {
     ...
     *                If a Winsock error occurs
     */
    protected static native int BindSocket(int sockPtr, byte[] name, boolean fastAddName) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native int ListenSocket(int sockPtr, byte[] callerName) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native void ConnectSocket(int sockPtr, byte[] remoteName) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native int SendSocket(int sockPtr, byte[] buf, int off, int len) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native int ReceiveSocket(int sockPtr, byte[] buf, int off, int maxLen) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native int ReceiveLengthSocket(int sockPtr) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native int SetNonBlockingSocket(int sockPtr, boolean nonBlocking) throws WinsockNetBIOSException;
     ...
     *                If a Winsock error occurs
     */
    protected static native int SelectReceiveSockets(int sockCnt, int[] readSocksIn, int[] readSocksOut) throws WinsockNetBIOSException;
     ...
     * @throws Exception
     */
    public static native void Win32CloseEvent(int eventHandle) throws Exception;
     ...
     * @exception WinsockNetBIOSException
     */
    public static native void WinsockCloseEvent(int eventHandle) throws WinsockNetBIOSException;
     ...
     * @throws WinsockNetBIOSException
     */
    public static native void WinsockEventSelect(int sockPtr, int eventHandle, int eventMask) throws WinsockNetBIOSException;
     ...
     * @throws WinsockNetBIOSException
     */
    public static native int WinsockEnumNetworkEvents(int sockPtr, int eventHandle) throws WinsockNetBIOSException;
     ...
     * @throws WinsockNetBIOSException
     */
    public static native int getSocketReceiveBufferSize(int sockPtr) throws WinsockNetBIOSException;
     ...
     * @throws WinsockNetBIOSException
     */
    public static native void setSocketReceiveBufferSize(int sockPtr, int bufSize) throws WinsockNetBIOSException;
     ...
     * @throws WinsockNetBIOSException
     */
    public static native int getSocketSendBufferSize(int sockPtr) throws WinsockNetBIOSException;
     ...
     * @throws WinsockNetBIOSException
     */
    public static native void setSocketSendBufferSize(int sockPtr, int bufSize) throws WinsockNetBIOSException;

    /**
     * Wait for a network address change event, block until a change occurs or the Winsock NetBIOS interface is shut down
     */
    ...
    static {
        ...
        final String dllName = X64.isWindows64() ? "Win32NetBIOSx64" : "Win32NetBIOS";
