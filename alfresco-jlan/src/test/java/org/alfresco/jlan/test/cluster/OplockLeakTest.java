	...
	public TestResult runTest( int threadId, int iteration, DiskSession sess, StringWriter log) {
		...
		try {
			...
			if ( threadId == 1) {
				...
				oplockFile = cifsSess.NTCreateWithOplock( testFileName, WinNT.RequestBatchOplock + WinNT.RequestExclusiveOplock, oplockHandler, AccessMode.NTReadWrite, NTFileAttributeType.Normal.getFlag(),
				...
				try {
					oplockFile = cifsSess.NTCreate( testFileName, AccessMode.NTReadWrite + AccessMode.NTReadAttrib, NTFileAttributeType.Normal.getFlag(), SharingMode.READWRITEDELETE, NTOpenAction.OVERWRITE_IF.getValue(), 0, 0);
